...
- **不要在 `.markdown-body` 的 `font-family` 里挂任何 emoji 字体**（`"Apple Color Emoji"` / `"Noto Color Emoji"` 都不行）。WeasyPrint+Pango 在 font-family 末尾出现 emoji 字体时会把数字与 CJK 边界的 advance width 算错——`"0 维"` / `"99%"` 这种位置会被撑出近 4 个字符宽的空格，看着像被 justify 过。emoji 字形（☑ ✅ ❌ 等）通过 fontconfig 的系统级 fallback 命中，**不需要**显式列在 CSS 字体栈里。
- **emoji fallback 必须落到"轮廓"字体上（Symbola），系统里不能装 Noto Color Emoji**——Noto Color Emoji 是 CBDT/CBLC **位图**字体（无 glyf 轮廓表），WeasyPrint 画不了位图字形：字宽照排（留出一段空白）、字形却被画成页面左上角原点处的微小色斑，正文里看起来就是 **✅ / ❌ 整个消失**（曾踩坑：全部章节 PDF 的 ❌✅ 都隐形了很久才被发现；PDF 文本层抽取得到字符、pdffonts 也显示字体已嵌入，唯独视觉上没有，极难排查）。而且 fontconfig 对 emoji-presentation 码点会优先匹配 "emoji" 泛型族（即 Noto Color Emoji），**只装 Symbola 不卸载 Noto Color Emoji 是赢不了的**。`install.sh` 已固化为**三层防御**（曾二次踩坑：卸载命令在沙箱环境里被 `>/dev/null 2>&1 || true` 静默吞掉失败，PDF 又隐形了一轮才被发现）：① 装 `fonts-symbola`（轮廓格式、全符号覆盖）并卸载 `fonts-noto-color-emoji`；② 写入 fontconfig 拒选规则 `/etc/fonts/conf.d/99-reject-bitmap-emoji.conf`——就算包还在或被其他工具装回，fontconfig 也不再暴露该字体（`fc-list` 都看不到），Pango 永远选不中；③ install.sh 末尾**大声验证**（fc-list 仍能看到该字体就 exit 1），`render.py` 启动时也做同样的运行时守卫，检测到位图字体直接中止而不是产出隐形 PDF。✅❌⚠ 渲染为 Symbola 的单色轮廓字形——没有彩色，但清晰可见。
