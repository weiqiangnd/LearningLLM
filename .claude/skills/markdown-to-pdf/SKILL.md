...
- **emoji fallback 必须落到"轮廓"字体上（Symbola），系统里不能装 Noto Color Emoji**——Noto Color Emoji 是 CBDT/CBLC **位图**字体（无 glyf 轮廓表），WeasyPrint 画不了位图字形：字宽照排（留出一段空白）、字形却被画成页面左上角原点处的微小色斑，正文里看起来就是 **✅ / ❌ 整个消失**（曾踩坑：全部章节 PDF 的 ❌✅ 都隐形了很久才被发现；PDF 文本层抽取得到字符、pdffonts 也显示字体已嵌入，唯独视觉上没有，极难排查）。而且 fontconfig 对 emoji-presentation 码点会优先匹配 "emoji" 泛型族（即 Noto Color Emoji），**只装 Symbola 不卸载 Noto Color Emoji 是赢不了的**。`install.sh` 已固化处理：装 `fonts-symbola`（轮廓格式、全符号覆盖）+ 卸载 `fonts-noto-color-emoji`，✅❌⚠ 渲染为 Symbola 的单色轮廓字形——没有彩色，但清晰可见。
