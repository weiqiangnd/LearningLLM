...
| 12 论文串读：GPT 系列演进与 LLaMA / Qwen 现代改动 | 顺两条时间线做一次论文串读。第一条线 GPT-1（decoder-only + 预训练 / 微调范式）→ GPT-2（规模拉大、Pre-LN 定型、zero-shot）→ GPT-3（175B、few-shot 上下文学习涌现），看 decoder-only 路线如何从「一种选择」变成「事实标准」——关键词是「骨架 + 规模」，三代参数量涨约 1500 倍而架构基本冻结，并点明「GPT-3 → ChatGPT 那一步改的是训练流程（SFT + RLHF）而非架构」。第二条线把原版 Transformer → LLaMA / Qwen 的现代改动（RoPE / RMSNorm / SwiGLU / GQA / 可选 MoE，外加去 bias、QK-norm、weight tying）逐项对照拆解，配一张「零件出处与时间差」时间轴讲清这些零件各自出自哪篇更早的论文、从「有论文」到「成为标配」为何常隔两三年，再凑成一张零件对照矩阵标出每项「首次换上」的那一代；这些零件第 4-9 章已介绍过，本章重在演进叙事与「读 config 反推零件谱系」的方法。实战全程 CPU：AutoConfig 读 GPT-2 与 Qwen3-8B 的 config，把论文改动与 config 字段一一对上，画参数量增长图、打印零件对照矩阵。 | [文档](./src/12-论文串读-GPT系列与现代改动.md) · [ipynb](./src/12.ipynb) · [OpenInColab](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/12.ipynb) | ✅ |
