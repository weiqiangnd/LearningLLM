...
**先修章节并不要求一次读完**：建议读者在主线**首次用到**对应背景知识时再回头补齐——下方各阶段会在这些"首次出现点"用 `〔先修〕` 标注，已经熟悉的可跳过。
...
02  生成参数与采样策略：logits / softmax / temperature / top-p/top-k / 思考模式 ← 已完成
   〔先修〕首次用到 softmax 与概率分布——若不熟悉，建议先读 P03。
```
...
04  Embedding 与位置编码：token embedding / sinusoidal / learned / RoPE / ALiBi
   〔先修〕首次密集用到张量 shape / broadcasting / autograd——若不熟悉，建议先读 P01。
...
10  自回归语言建模目标、cross-entropy loss、teacher forcing 与推理采样的关系
   〔先修〕首次用到极大似然估计、熵 / 交叉熵 / KL 散度——若不熟悉，建议先读 P03。
...
13  从零实现 mini-GPT：tiny-shakespeare 上完成训练 → 生成闭环
   〔先修〕首次走完「前向 + 反向 + 优化器」完整训练循环——若不熟悉，建议先读 P02 与 P04。
```
...
31  RLHF 全景：PPO 在 RM 上的应用、KL 约束、reward hacking ⚡
   〔先修〕首次用到 MDP / policy gradient / PPO——若不熟悉，建议先读 P05。
