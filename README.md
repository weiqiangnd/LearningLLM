...
| 11 论文精读：Attention Is All You Need 逐节解读 | 逐节精读 2017 年奠基论文《Attention Is All You Need》，把第 3-10 章介绍过的零件（scaled dot-product attention、多头、位置编码、FFN、残差、训练目标）对照论文每一节重读：从引言里 RNN 的顺序瓶颈，到第 3 节逐子节架构（原版 Post-LN、注意力三处用法、embedding 乘 √d_model 等易漏细节）、第 4 节 self-attention vs RNN vs CNN 复杂度表、第 5 节 Adam + warmup + label smoothing 训练配方、第 6 节读消融表的方法论；并贯穿一条「2017 → 今天」对照线（Post-LN→Pre-LN、LayerNorm→RMSNorm、ReLU→SwiGLU、sinusoidal→RoPE、MHA→GQA、两栈→decoder-only），为第 12 章论文串读铺路。实战全程 CPU 复现论文核心公式与图表：√d_k 缩放防 softmax 饱和、sinusoidal 热力图、自注意力对 RNN 的并行优势、warmup 调度曲线、label smoothing、数清 base/big 参数量对上 Table 3，并读 Qwen3-8B config 落到真实数字。 | [文档](./src/11-论文精读-Attention-Is-All-You-Need.md) · [ipynb](./src/11.ipynb) · [OpenInColab](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/11.ipynb) | ✅ |
| 12 论文串读：GPT 系列演进与 LLaMA / Qwen 现代改动 | 顺两条时间线做一次论文串读。第一条线 GPT-1（decoder-only + 预训练 / 微调范式）→ GPT-2（规模拉大、Pre-LN 定型、zero-shot）→ GPT-3（175B、few-shot 上下文学习涌现），看 decoder-only 路线如何从「一种选择」变成「事实标准」——关键词是「骨架 + 规模」，三代参数量涨约 1500 倍而架构基本冻结，并点明「GPT-3 → ChatGPT 那一步改的是训练流程（SFT + RLHF）而非架构」。第二条线把原版 Transformer → LLaMA / Qwen 的现代改动（RoPE / RMSNorm / SwiGLU / GQA / 可选 MoE，外加去 bias、QK-norm、weight tying）逐项对照拆解，配一张「零件出处与时间差」时间轴讲清这些零件各自出自哪篇更早的论文、从「有论文」到「成为标配」为何常隔两三年，再凑成一张零件对照矩阵标出每项「首次换上」的那一代；这些零件第 4-9 章已介绍过，本章重在演进叙事与「读 config 反推零件谱系」的方法。实战全程 CPU：AutoConfig 读 GPT-2 与 Qwen3-8B 的 config，把论文改动与 config 字段一一对上，并画一张 GPT-1/2/3 参数量增长图。 | [文档](./src/12-论文串读-GPT系列与现代改动.md) · [ipynb](./src/12.ipynb) · [OpenInColab](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/12.ipynb) | ✅ |
| 13 从零实现 mini-GPT：tiny-shakespeare 上完成训练 → 生成闭环 | 阶段 2 的收官实战：从 1.1 MB 的 tiny-shakespeare 出发，不调用任何现成模型类，自己写字符级 tokenizer、滑窗取批（x / y 错开一位，一条样本就是 L 条监督）、搭 4.76 M 参数的 mini-GPT（RMSNorm + RoPE + SwiGLU + Pre-LN，外加 weight tying、初始化缩放与 dropout）、写训练循环（loss 拍平、AdamW + warmup + cosine、梯度裁剪、看 val loss 读过拟合）与采样循环。实战在 T4 上 3 分钟训完 3000 步：loss 从 ln(65)=4.17 掉到 1.12，生成从纯乱码变成带角色名与分行的莎士比亚腔文本，最后量出朴素 generate 循环约 98.4% 的计算是在重算前缀——正好引出第 14 章的 KV cache。 | [文档](./src/13-从零实现mini-GPT.md) · [ipynb](./src/13.ipynb) · [OpenInColab](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/13.ipynb) | ✅ |
...
〔预备知识〕13 章首次**完整**用上一整套 LLM 训练配方：AdamW 的解耦式 weight decay、warmup + cosine 学习率调度、梯度裁剪——若不熟悉，建议先读 P04。
