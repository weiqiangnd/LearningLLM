...
| 04 Embedding 与位置编码：token embedding / word2vec / sinusoidal / learned / RoPE / ALiBi | 从「整数 id 喂不进网络」讲起，对比 one-hot 与稠密 embedding，说清 token embedding 本质是「one-hot 乘 embedding 矩阵 = 按行查表」、embedding 矩阵 [V, d] 与输出层 lm_head [d, V] 互为转置可共享权重（weight tying）；借 word2vec（分布假说、CBOW / skip-gram + 负采样）讲透 embedding 为什么带语义结构、king − man + woman ≈ queen 为何成立，并厘清它与 LLM embedding 的异同（静态词向量 vs 端到端学、之后被 attention 上下文化）；论证自注意力对 token 顺序「视而不见」（置换等变）、从而位置编码是刚需，梳理「改输入（绝对）vs 改注意力（相对）」两条注入路线；逐个推导 sinusoidal（正余弦公式、相对位置的线性表达）、learned（位置表、上下文焊死在 max_len）、RoPE（旋转 q/k 让点积只依赖相对位置 m−n、base 是扩上下文的旋钮）、ALiBi（按距离给分数减线性惩罚）四种位置编码的原理与取舍；实战用小张量实现 embedding 查表与 weight tying、训一个 word2vec 验证经典类比、代码实证自注意力的置换不变性、画 sinusoidal 热力图、实现并验证 RoPE 的相对位置性质、构造 ALiBi 偏置矩阵，并加载 Qwen3 config 看真实的 rope_theta。 | [文档](./src/04-Embedding与位置编码.md) · [ipynb](./src/04.ipynb) · [OpenInColab](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/04.ipynb) | ✅ |
