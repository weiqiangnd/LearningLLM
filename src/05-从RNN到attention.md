...
前四章我们把「输入表示」这条链路补齐了：文本切成 token（第 3 章）、token 查表变成带语义、带位置的向量（第 4 章）。第 4 章结尾还埋了个钩子——**自注意力对顺序视而不见，所以要外挂位置编码**。但我们一直没正面回答一个更根本的问题：**attention 这东西到底是怎么冒出来的？为什么大家会想到「让模型去注意输入的某些部分」？**

这一章就来还这笔账。我们不直接跳到 Transformer 的自注意力，而是回到 2014 年那个真实的历史现场：当时做机器翻译的主力是 **RNN**（循环神经网络）搭起来的 **seq2seq**，它有个绕不过去的硬伤——**把整句话压进一个定长向量、句子一长就记不住开头**。attention 正是为了补这个窟窿被发明出来的。理解了这段「病因 → 药方」的来龙去脉，你再看第 6 章的 scaled dot-product attention，就不会觉得 Q/K/V 是天上掉下来的玄学，而是水到渠成。
...
- 实战亲手训一个带 attention 的 seq2seq 做「数字串反转」，**把对齐矩阵画出来看 attention 在「看」哪里**，再用一组「准确率随长度变化」的对比，**亲眼实证那个瓶颈**
...
- [八、实战：训一个带 attention 的 seq2seq，看对齐矩阵](#八实战训一个带-attention-的-seq2seq看对齐矩阵)
  ...
  - [8.5 瓶颈实证：准确率随长度的对比](#85-瓶颈实证准确率随长度的对比)
- [九、关键概念回顾](#九关键概念回顾)
- [十、本章小结](#十本章小结)
...
1. **能吃变长输入**——「你好」是 2 个 token，「今天天气真不错」是 7 个，模型不能要求输入必须是固定长度。
2. **尊重顺序**——「狗咬人」和「人咬狗」不是一回事（这点第 4 章反复强调过）。
...
其中 $\mathbf{h}\_t \in \mathbb{R}^{d_h}$ 是 $d_h$ 维隐藏状态， $\mathbf{x}\_t$ 是当前输入向量， $W_{hh}$ 把旧隐藏映射到新隐藏、 $W_{xh}$ 把输入映射到隐藏， $\tanh$ 是逐元素的非线性激活（把数值压到 $[\thinspace {-1},\ 1\thinspace ]$ ，防止反复相乘后爆炸）。初始记忆 $h_0$ 一般置零。

这里有两个要点，请务必记住，它们是后面所有讨论的地基：

- **同一套权重，反复使用**。 $W_{hh}, W_{xh}, \mathbf{b}$ 在每一个时间步都是**同一份**——不管序列多长，参数量不变。这就是「循环」二字的含义：把同一个计算单元在时间轴上反复套用。正因如此，RNN 天然吃得下变长输入（第 1 条要求满足）。
...
- **编码器（encoder）**：一个 RNN，把源句一个词一个词读完，最后吐出它的**末隐藏状态**，记作 **context 向量** $\mathbf{c}$ 。这个 $\mathbf{c}$ 被当作「整句话的浓缩表示」。
...
$$
\mathbf{s}_t = f(\mathbf{s}_{t-1},\ \mathbf{y}_{t-1},\ \mathbf{c}), \qquad P(y_t \mid y_{\lt t}) = \text{softmax}(W_o\thinspace \mathbf{s}_t)
$$
...
attention 每一步干的事，可以拆成**雷打不动的三步**，后面 Bahdanau、Luong 乃至第 6 章的自注意力，都是这套骨架，只是细节实现不同：
...
- $W, U$ ：两个可学习的线性变换矩阵，分别作用在 query 和 $h_j$ 上，把它俩投到同一个空间好相加。 $\mathbf{v}$ ：一个可学习的向量，把 $\tanh$ 之后的隐藏向量压成**一个标量分数**。
...
本章实战故意选了一个**对齐图形特别醒目**的玩具任务：**把数字串反转**（输入 `3 1 4 1 5`、输出 `5 1 4 1 3`）。因为逆序任务里，解码第 $t$ 步必须去对齐源句的第 $(L-1-t)$ 个位置，所以学好的对齐矩阵会是一条干净的**反对角线**（从右上到左下）。实战中的 Cell 9 会把它画出来，你能一眼看到 attention 真的学会了「回看正确的输入位置」。
...
- **dot**：最省事——query 和 $h_j$ 直接做**点积**。点积大说明两个向量方向接近、即「匹配」（点积衡量相似度，这点第 4 章在 word2vec 与位置编码里已经反复用过）。**前提是 query 和 $h_j$ 维度相同**。
...
光看公式不过瘾。这一节我们**亲手**搭一个带 Bahdanau attention 的 GRU seq2seq，在「数字串反转」任务上训练，干两件事：

1. **把对齐矩阵画出来**——肉眼确认 attention 学到了正确的（反对角线）对齐。
2. **实证瓶颈**——再训一个**不带 attention** 的 baseline（context 固定为 encoder 末隐藏），对比两者「准确率随源句变长」的曲线，亲眼看到 baseline 怎么随长度崩掉、attention 怎么稳住。

> 完整可运行代码在配套的 `05.ipynb`。下面把每个 cell 的核心代码贴出来讲；**这些代码片段与 ipynb 里的对应 cell 逐字一致**，你既可以在 Colab 里 Run All，也可以照着这里一段段读。全程 CPU、约 2–4 分钟。

### 8.1 任务、数据与环境

**Cell 0 / 1** 是雷打不动的环境自检 + 装依赖（本章只额外用到 `matplotlib` 画图）。环境自检照例打印 PyTorch 版本与 CUDA 可用性——但本章用不到 GPU，CPU 即可：
...
> **这个坑值得停一下**：一个 batch 里序列长短不一，得右侧补 `PAD` 凑成矩形。如果直接把补完 `PAD` 的整条喂 GRU，GRU 会「认认真真」地把那些 `PAD` 也当输入处理，于是末隐藏 `h_n` 变成了「跑完一串 PAD 之后」的状态——而且补多少 PAD 取决于 batch 里最长那条有多长，导致**同一条短序列在不同 batch 里拿到不同的 `h_n`**，模型直接学歪（实测：不加 pack 时短序列准确率会异常地低）。`pack_padded_sequence` 告诉 GRU 每条的真实长度、让它在真实末 token 处停下，`h_n` 就稳了。这是 RNN 处理变长 batch 的标准姿势。
...
**Cell 6** 是解码器，用一个 `use_attention` 开关同时实现「带 attention」和「baseline」两种模式——这样两个模型共用一份代码、对比才公平。带 attention 时每步动态算 `context`；baseline 则 `context` 恒为 encoder 末隐藏（那个被诟病的固定瓶颈）：
...
**预期现象**：带 attention 的模型 loss 很快降到 0.001 量级；baseline 的 loss 则卡在 0.1–0.2 下不去——因为它得把整条源序列硬塞进一个固定向量，长序列上力不从心。

**Cell 9** 是本章的高光：取一条长度 8 的样本，把带 attention 模型的对齐矩阵画成热力图：
...
**预期现象**：打印出的「模型预测」和「目标(逆序)」完全一致；热力图上**亮点连成一条从右上到左下的反对角线**——解码第 0 步盯着源句最后一位、第 1 步盯倒数第二位……正是逆序任务该有的对齐。这就是「看得见的 attention」：模型没人教，自己学会了每一步该回看输入的哪个位置。
...
```python
# ============================================================
# Cell 10: 瓶颈实证——准确率随序列变长，baseline 掉得比 attention 快（对应第 2.3 / 6 节）
# ============================================================
...
这张表（和对应曲线）就是本章最有说服力的一句话：**短句两者都行；句子一长，固定 context 的 baseline 准确率断崖式下跌，而 attention 几乎纹丝不动。** 你亲手把「2.3 节的瓶颈」和「attention 怎么治它」一次性验证了——这正是 2014 年 attention 被发明出来的全部动机。
...
| **context 向量 $\mathbf{c}$** | 编码器末隐藏，被当作「整句话的浓缩」交给解码器——瓶颈就出在这「一个定长向量」上 |
...
- **attention** 对症下药：**不再压扁**，保留 encoder 所有隐藏态；解码每一步**动态**算一个当步专属的 $\mathbf{c}\_t$ 。机制是雷打不动的**三步套路**——打分（对齐）→ softmax → 加权求和。
...
- 实战我们亲手训了带 attention 的 GRU seq2seq 做数字串反转，**把对齐矩阵画成了清晰的反对角线**（看见 attention 学到的对齐），又用一条「准确率随长度变化」的对比曲线，**亲眼实证了无 attention 的 baseline 怎么随长度崩、attention 怎么稳住**——把 attention 的来历从头到尾验证了一遍。
