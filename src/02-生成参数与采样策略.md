...
- $T \to 0^+$ ：除最大 logit 外的所有项被指数级压扁，最大那一项独占归一化后的概率，分布**退化到 argmax**——这就是贪心解码
- $T = 1$ ：恢复模型自己的"原汁原味"分布
- $T \to \infty$ ：所有 logits 被压平，分布趋近**均匀分布**——完全随机

#### 一个最小数值例子

假设词表只有 4 个 token（示例情形），logits 是 $\boldsymbol{\ell} = [\thinspace 2.0,\ 1.0,\ 0.5,\ 0.1 \thinspace]$ 。算一下不同 $T$ 下的概率分布（两位小数）：
...
Qwen3 在 chat 数据上学到的是"这一轮该说完时就输出 `<|im_end|>`"，因此对话场景下真正的终止符是 `<|im_end|>` 而不是通用的 `<|endoftext|>`。Qwen3 直接在 `tokenizer_config.json` 里把 `eos_token` 静态配置成了 `<|im_end|>`——所以 `tokenizer.eos_token_id` 拿到的就是它；仓库的 `generation_config.json` 则默认把 `<|im_end|>` 和 `<|endoftext|>` **都**列进了终止 token 列表。第 1 章里我们写过：
...
# ============================================================
# 词表只有 4 个 token 的示例情形，logits = [2.0, 1.0, 0.5, 0.1]
...
> ⚠️ **耐心提示**：思考模式 `max_new_tokens=2048`，T4 上 4-bit Qwen3-8B 速度约 10–15 tok/s，**单次推理需要 2–4 分钟**；本章 `chat()` 用的是非流式生成，期间 cell 不会有任何中间输出——看着像卡住，其实在算，等一会儿就行。想看 GPU 是否还在工作，可在另一个 cell 跑 `!nvidia-smi` 看占用。
...
- 非思考模式通常能直接给出正确答案（Qwen3-8B 这种规模已经能稳定应付二元一次方程组），或简短列一下方程但跳步，过程较短
