...
**纯 SGD 的更新方向被垂直分量主导**（垂直方向是沿谷方向的 10 倍），所以在 $y$ 方向反复横跳；**Momentum 把更新方向"扭"到沿谷方向**（沿谷方向反过来变成垂直方向的近 2 倍）——这就是上图蓝线能沿着谷底滑行的原因。
...
- 抵达最优点附近后，图里仍能看到 $y$ 在小幅来回——这是 $\hat m_y$ 残留着早期大 $|g_y|$ 的"记忆"，要等它和 $\sqrt{\hat v_y}$ 一起衰减才彻底安静（在这个确定性梯度的示例里，update 终究会随 $|g| \to 0$ 一起趋于 $0$ ）。**真实训练里**还会有 mini-batch 噪声给 update 留一个 $\approx \eta$ 的"地板"——所以训练后期通常配合 cosine 退火（见第 8 节）让 lr 自己变小，把这个地板一起压下去。
...
但**在 Adam 里这两种实现并不等价**：Adam 会对 $g$ 做 $\sqrt{\hat v}$ 归一化，如果 weight decay 也走 $g$ 这条路，那它会被 $\sqrt{\hat v}$ 一起缩放——**历史梯度量级大的参数，weight decay 反而被压小**（ $\hat v$ 估计的是梯度平方的滑动平均，梯度越大分母越大，正则项被除得越狠）。这跟"weight decay 应该平等地拉每个参数往 0"的初衷不一致。
...
**为什么不 warmup 就容易出 NaN？** 随机初始化时第一个 batch 算出的梯度方向接近随机噪声、模长又往往偏大；如果立刻用峰值 lr 走一步，参数会被推到一个偏离初始化良态分布的位置——下一次前向里某些通道的激活值急速放大、中间层数值溢出半精度（fp16 / bf16）表示范围，softmax / cross-entropy 一算就输出 NaN；而 NaN 一旦出现就会沿计算图传染（任何含 NaN 的算术结果还是 NaN），后续整条 loss 全是 NaN，再也回不来。warmup 把前若干 step 的 lr 压到接近 0，每一步参数挪动都很小，这条"参数跳飞 → 激活溢出 → NaN"的链就被掐在第一环。
...
- **主流 LLM 预训练（GPT-3、LLaMA、Qwen 等）清一色采用 cosine**——这些论文并没有公布与 linear / step 的消融对比（Kaplan et al. 2020 甚至报告最终 loss 对调度形状并不敏感），但 cosine 平滑、无拐点、只需定起止两端，已经成为事实默认
...
HuggingFace `transformers` 里就有现成实现（注意选对函数：名气更大的 `get_cosine_schedule_with_warmup` 是**一路衰减到 0**、没有 min_lr 参数的版本；要复现上图那条 10% 下界，得用带 `min_lr` 的这个）：
...
scheduler = get_cosine_with_min_lr_schedule_with_warmup(
    ...
    min_lr_rate=0.1,              # η_min = 10% · η_max —— 对应上图的下界
)
...
**`scheduler.step()` 必须在 `optimizer.step()` 之后调**——PyTorch 1.1 起的约定。写反会怎样？schedule 会被**提前一步消耗**：第一个 lr 值直接被跳过（PyTorch 官方文档的原话是 "skip the first value of the learning rate schedule"），在 warmup 场景里意味着前几步用的 lr 比设计值**偏大**——而 warmup 防的恰恰是开头 lr 过大，写反等于削弱了 warmup 本身。
...
2. **lr range test**：从一个极小 lr（如 $10^{-7}$ ）开始，几百步内指数升到 $10^{-1}$ ，画 loss vs lr——曲线形态通常是「平 → 陡降 → 谷底 → 爆炸」，取**下降最陡那一段**的 lr、或者保守一点取 loss 开始飙升（发散点）前约 1/10 的位置当 peak（Smith 2015 / fastai 的标准做法）
