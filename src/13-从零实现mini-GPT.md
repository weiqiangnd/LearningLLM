...
第 3-12 章我们做了四件事：把零件一个个拆开（tokenizer、embedding、attention、FFN、归一化）、把它们总装成 decoder-only 骨架（第 9 章）、再讲清了这个骨架靠什么目标训练（第 10 章），最后顺着论文把这套东西的来历读了一遍（第 11-12 章）。零件、总装、目标、源流，四件事齐了——**唯独还差一件：真的训一个出来**。

这一章就补上这最后一步。我们要从一份 1.1 MB 的莎士比亚剧本文本出发，**不调用任何现成的模型类**，一路走完：

> 语料 → 切成 token → 编码成 id → 切成训练批次 → 搭模型 → 训练循环 → 采样生成 → 存档

跑完你会看到一个约 4.8 M 参数的小模型，从最初吐出的纯乱码，变成能写出这样的东西：分行、有角色名后跟冒号、有像英文单词的词、有莎士比亚腔的句读。它当然写不出真正通顺的句子（4.8 M 参数、1 MB 数据，做不到），但**这条「训练 → 生成」闭环上的每一个环节，你都亲手接过一遍**——这正是本章的目的。
...
差的是**从「能前向」到「能学会」之间的那一段**。第 9 章那个模型跑的是随机数据、学的是「把序列右移一位」这种玩具任务，目的只是验证形状守恒；它没有真实语料、没有验证集、没有学习率调度、没有采样生成，也没有存盘。而这一段恰恰是实际工作里最容易出错的地方——数据错位一格、loss 拍平时维度对不上、生成时忘了裁上下文、训练完忘了切 `eval()`，这些坑没有一个能靠读论文避开，只能自己踩一遍。

所以本章的定位是**收官实战**：把前面所有零件重新写一遍（代码自包含，不依赖前面章节的 notebook），接上真实数据和完整训练循环，让你看到 loss 从 4.17 一路掉下来、看到生成的文本一点点从乱码里长出结构。
...
这张图值得多看两眼，因为它把第 10 章那句抽象的话变成了具体的数据流：**训练和推理面对的是同一个概率分布 $P_\theta(\text{下一个 token} \mid \text{前文})$** ——训练用真实语料把这个分布拟合出来，生成则按这个分布一个 token 一个 token 地采样。本章第 2-5 节走上面那条环，第 6 节走下面那条环。

### 1.3 mini-GPT 与 Qwen3-8B：一张规模对照表

动手之前先建立规模感，免得对结果有不切实际的期待。下面是本章要造的东西和一个真实开源大模型的对照：
...
差距是四到七个数量级，但请注意一件事：**这两列的「零件」是同一套**——都是 RoPE + RMSNorm + SwiGLU + Pre-LN 的 decoder-only 骨架，都用 cross-entropy 训、都用温度 / top-k 采样。**变的只是尺寸和数据量**。这正是第 9 章说的「形状守恒让放大退化成调超参」在实践中的样子：你在本章写下的 `MiniGPT` 类，把几个数字改大，就是工业级模型的结构。
...
**tiny-shakespeare** 是一份把莎士比亚全部剧作拼在一起的纯文本，约 **1,115,394 个字符**（1.1 MB），最早由 Andrej Karpathy 在 char-rnn 项目里整理出来，之后成了「验证一个语言模型实现对不对」的事实标准玩具数据集。它长这样：
...
因为**本章的瓶颈不是压缩率，是模型容量**。理由说白了就一条：**词表大小 $V$ 直接决定两端那两张大表的尺寸**。embedding 是 $V \times d$ 、lm_head 是 $d \times V$ ，按本章 $d_{\text{model}} = 256$ 算：

| tokenizer | 词表 $V$ | embedding 参数量 | 占本章模型（4.76 M）的比例 |
|-----------|---------|-----------------|--------------------------|
| 字符级（本章） | 65 | 16,640 | 0.35% |
| Qwen3 的 BBPE | ≈ 151k | ≈ 38.9 M | 800%——是整个模型的 8 倍 |
...
- **每个 token 语义更稀薄**：模型得先花力气学会「字母怎么拼成单词」这件对 BPE 模型来说免费的事。这也是为什么本章模型的输出会有拼错的单词——它是一个字母一个字母猜出来的。
...
这里的关键是**按位置前后切、不打乱**。为什么不像做分类任务那样随机划分？因为语言模型的样本是**从连续文本里滑窗切出来的**——如果先随机打乱再切，验证集里的一段话很可能与训练集里某段**有重叠**（同一句话的两个错位窗口），模型只是背下来了却看不出来，验证集就失去了「衡量泛化」的意义。按位置切开，验证集是模型**完全没见过的后 10% 剧本**，这才测得准。

> 顺手提一句：切完之后 `val_data` 是剧本的最后 10%，与前面 90% 属于不同的剧目，用词和人物都不同。所以本章后面会看到 **val loss 始终比 train loss 高一截**（实测从 250 步的 0.15 慢慢扩大到 3000 步的 0.34），一直没追平——这既有过拟合的成分，也有「前后文本分布本就不同」的成分。这在小数据集上是正常现象，不用当成 bug。

---

## 三、从长序列到训练批次

数据现在是一条一百多万长的 id 序列，而模型一次只能吃 `[B, L]` 的一批。中间这一步转换很短，但**是整个训练里最容易错一格的地方**。
...
`torch.randint(len(d) - L - 1, (bs,))` 里那个 `- L - 1` 是**边界保护**：起点最大只能取到「离末尾还剩 $L+1$ 个 token」的位置，否则 `d[i+1 : i+L+1]` 会切出不足 $L$ 个元素，`torch.stack` 立刻报形状不一致。这类 off-by-one 是新手写数据管线时最常见的报错，写的时候先想清楚「最后一条样本的起点在哪」就不会错。
...
- **随机起点带来对齐上的多样性**。如果固定切成 `[0:128]`、`[128:256]`、`[256:384]`……那么某个词永远只会出现在窗口的固定位置上；随机起点让同一段文本在不同批次里以不同的对齐方式出现，相当于免费的数据增强，也让模型对「一句话从窗口中间开始」这种情形更鲁棒。
- **实现上简单得多**。不用维护「遍历到哪了」「这一轮结束了没」的状态，不用处理最后一个不满 $L$ 的残块，随手 `randint` 就完事。

代价是**epoch 这个概念变模糊了**：随机采样时同一段文本可能被采到多次、也可能一次没采到，不再是严格的「每条样本恰好过一遍」。所以本章的训练循环按**步数（step）** 而不是 epoch 计数——这也正是大模型预训练的通行做法（数据以 T 计，本来也只过一遍甚至不到一遍，epoch 这个单位没什么意义）。
...
这是自回归语言建模最划算的地方（第 9 章第 4.1 节说的「训练信号最密」）：**数据里的每一个 token 都是一条免费的监督信号**，不需要任何人工标注。也正因如此，「一次前向覆盖多少 token」是衡量训练吞吐的核心指标——大模型训练里说的 tokens/s，数的就是这个。
...
| 组件 | 计算式 | 参数量 | 占比 |
|------|--------|--------|------|
| token embedding（与 lm_head 共享） | $V \times d = 65 \times 256$ | 16,640 | 0.3% |
...
- **两端的词表几乎不占参数**（0.3%），这正是第 2.2 节选字符级的直接结果。作为对比，Qwen3-8B 的 embedding + lm_head 加起来是 1.24 B，占整模型的 15%。
...
这一条的来历是这样： $N$ 层模型里，每层有 **2 个**子层往残差流上加东西（注意力一个、FFN 一个），一共 $2N$ 个增量。若把这些增量近似看成互相独立、方差都是 $\sigma^2$ ，那么累加之后的方差约是 $2N\sigma^2$ ，**标准差随 $\sqrt{2N}$ 增长**。想让残差流在输出端的尺度与输入端持平，就把每个增量的初始尺度按 $1/\sqrt{2N}$ 压回去。GPT-2 论文里那句「按 $1/\sqrt{N}$ 缩放残差层的初始化」说的就是这件事（它的 $N$ 指残差路径的总条数，本章按 $2N$ 条算）。
...
模型输出的 logits 形状是 `[B, L, V]`，标签是 `[B, L]`，而 `F.cross_entropy` 要的是「一批样本，每个样本一个类别分布」，即 `[N, C]` 配 `[N]`。所以要把前两维**拍平成一维**：
...
优化器直接用 P04 的结论——**AdamW**，这是 LLM 训练的事实默认。三个参数说明一下：
...
- **`weight_decay=0.1`**：比一般视觉任务（1e-4 量级）大得多，也是 LLM 的常见取值。AdamW 的解耦式 weight decay（P04 第 5 节）让这个值可以直接理解为「每步把权重往零拉多少」。
...
阈值 1.0 是通行取值。你会在实战里看到一个典型现象：**第 0 步的梯度范数就有 10.4，开头 30 来步连续触发裁剪，随后迅速掉到 1.0 以下、稳定在 0.45 附近**（3000 步里总共只有 33 步被裁，全部落在前 52 步内）——裁剪主要在保护训练早期那段最脆弱的时期。
...
本章的实际情况值得预先说明：3000 步内两条都还在降（验证 loss 从 1.83 一路降到 1.46），**没有掉头**，所以还谈不上「必须停」；但**两者的差距在稳步拉大**（250 步时 0.15、1000 步时 0.21、3000 步时 0.34），这是过拟合正在积累的信号。这个差距主要来自两件事——**一是**第 3.2 节算过的「语料被过了约 12 遍」，模型确实记住了一些训练片段；**二是**第 2.3 节说的，验证集是剧本的最后 10%，本身与训练部分的用词分布就有差异。真想把它压下去，正路不是调参，而是**换更大的数据集**——这也正是第 21 章 scaling law 要回答的问题：模型多大、数据多少才配得上。
...
| 调度 | cosine 退火 | 后期小步精修，是 LLM 的通行做法 |
...
- **RoPE 的 cos/sin 表只算到 `block_size`**（本章 128）。喂进更长的序列，`self.cos[:L]` 会取不满，直接报错。
...
| 设置 | 效果 | 本章的现象 |
|------|------|-----------|
| `temperature=0`（贪心） | 每步取概率最大的 token | 读着比采样版还「通顺」，但会不断绕回同一批句式（`the sea of the seas of the seas`）；从单个换行符起步时更极端，直接卡成无限换行 |
...
把这笔开销数清楚：从 1 个 token 生成到 128 个，累计前向的位置数是 $1 + 2 + \dots + 128 = 8256$ ，而真正新出现的位置只有 128 个——**约 98.4% 的计算是在重复上一步已经算过的东西**。序列越长，这个比例越高。
...
**Cell 5** 是全部零件：RMSNorm、RoPE、因果自注意力、SwiGLU、以及把它们裹上残差的 Block。这段代码和第 6-9 章一脉相承，区别只在于加了 dropout：
...
**预期现象**：文件约 **18 MB**（4.76 M 参数 × 4 字节 fp32 ≈ 19 MB）；重建后所有参数逐个相等；固定同一个随机种子时，两个模型生成的文本**完全一致**。
...
n = block_size
print(f"\n从 1 个 token 生成到 {n} 个：累计前向了 {sum(range(1, n + 1)):,} 个位置，"
      f"其中真正新出现的只有 {n} 个。")
...
**哪些是真的等价**：架构骨架、训练目标、loss 的算法、优化器与调度、采样策略——这几样本章写的和工业实现是同一回事，只是尺寸不同。你手上这份 `MiniGPT` 把几个数字改大、数据换成清洗好的语料，训练逻辑一行不改也是能跑的。
...
看清这张表，你对「大模型到底难在哪」也就有了实感：**难点从来不在架构本身**（本章 100 多行就写完了），而在数据、算力、工程效率，以及把 base 模型对齐成可用助手的那一整套后训练。
...
- 本章把第 3-12 章的知识**落成了一个能跑的东西**：从 1.1 MB 的莎士比亚文本出发，自己写 tokenizer、自己切批次、自己搭模型、自己写训练循环和采样循环，最后训出一个 **4.76 M 参数**的 mini-GPT，并让它写出带分行、带角色名、带标点的莎士比亚风格文本。全部代码约一百多行，不依赖任何现成的模型类。
...
- **和真实预训练比，本章等价的是「架构 + 目标 + 优化器 + 采样」，简化掉的是「数据工程 + 训练效率 + 推理工程 + 后训练」**。也就是说，大模型的难点从来不在架构本身（一百多行就写完了），而在数据、算力、工程，以及把 base 模型对齐成可用助手的那一整套流程——这些正是后面几个阶段要展开的内容。
