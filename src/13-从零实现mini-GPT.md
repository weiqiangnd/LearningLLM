...
>
> 〔预备知识〕本章首次把一整套 LLM 训练配方**完整**用起来——AdamW 的解耦式 weight decay、warmup + cosine 学习率调度、梯度裁剪（前面章节至多用到固定学习率的 Adam / AdamW，加一段线性 warmup）；训练循环本身仍是「前向 → 反向 → 优化器更新」那三步。若对优化器选型与学习率调度不熟悉，建议先读 P04（训练三步则见 P02）。
