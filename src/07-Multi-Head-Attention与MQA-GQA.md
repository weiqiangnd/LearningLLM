...
第 6 章我们把 Transformer 最核心的算子 **scaled dot-product attention** 从零搭完了：给定一个序列，每个位置作为 query 去和所有位置打分、缩放、softmax、加权求和，得到融合全局信息的新表示。那一章我们刻意只盯着**一个头**——把 batch 维 $B$ 和多头维 $H$ 都按下不表，只推一个 `[L, d_k]` 的序列。第 6 章结尾也留了话：真实模型用的从来不是「一个头」，而是把这套运算**并排复制成很多份**。
...
- [四、参数量与计算量：多头其实「不要钱」](#四参数量与计算量多头其实不要钱)
...
一个头、一个 $A$ 矩阵，逼着模型把这几种关系**揉成一个权重分布**——要么顾此失彼，要么折中得谁都不突出。更糟的是，softmax 加权求和本质是个**平均**操作：把多种该被分别强调的信息平均到一起，细节就被抹平了。第 6 章 Q4 提过「输出是 value 的凸组合」，凸组合就是加权平均，平均多了自然糊。
...
- $H$ 个头各自独立地打分、softmax、加权求和，产出 $H$ 个**互不相同**的注意力权重矩阵——头 1 可以专 attend 指代、头 2 专 attend 句法，互不干扰；
- 算完把 $H$ 个头的输出**拼回 $d$ 维**，再过一个线性层融合。

因为每个头只在 $d_k = d/H$ 维上算， $H$ 个头加起来的计算量**和一个 $d$ 维的大头基本持平**（第 4 节会算这笔账）——多头几乎是「免费」换来了「能同时关注多种关系」的表达力。下一节就把这套机制一步步拆开。
...
注意缩放因子是 $\sqrt{d_k}$ ——是**每个头的**维度 $d_k$ ，不是 $d_{\text{model}}$ 。这正是第 6 章 3.2 节那个论证的延续：每个头的点积在 $d_k$ 维上做，方差是 $d_k$ ，所以除以 $\sqrt{d_k}$ 把标准差拉回 1。
...
假设大投影之后某个头的 Q 已经算好，形状是 `[B=1, L=2, d_model=4]`，去掉 batch 维看这个 `[2, 4]` 矩阵：
...
再乘 $W_O \in \mathbb{R}^{4 \times 4}$ 得到最终 `[2, 4]` 输出——和输入 $Q$ 形状一致。整个过程里**唯一改变数据的是各头内部的 attention 和最后的 $W_O$ ，切头 / 合头本身只是搬运**。实战 7.3 节会用 `torch` 把这套 reshape / transpose 一行行打印形状给你看。
...
计算量同理。开销主要在 $Q K^\top$ 和 $A V$ 这两块矩阵乘上。单看 $Q K^\top$ ：单头（ $d_k = d_{\text{model}}$ ）要 $L^2 d_{\text{model}}$ 次乘加； $H$ 个头每个是 $L^2 d_k$ ，加起来 $H \cdot L^2 d_k = L^2 (H d_k) = L^2 d_{\text{model}}$ ——**完全相等**。多头把同样的计算量「分给」了 $H$ 个子空间，没有额外开销（多出来的只有一个 $W_O$ 投影，量级 $L d_{\text{model}}^2$ ，相对小）。

> 这就是多头设计最漂亮的地方：**几乎免费地**把「一种关注模式」升级成「 $H$ 种关注模式」。代价只是多了一个 $W_O$ ，换来的是表达力的实质飞跃。

### 4.2 那为什么不干脆用一个大头

既然参数量、计算量都一样，为什么不用一个 $d_k = d_{\text{model}}$ 的大头，省去切来切去？《Attention Is All You Need》原文给了答案，核心就一句：**单个大头里的 softmax 加权求和是一种「平均」，会把不同子空间该分别强调的信息抹平**（1.2 节那个「一支笔 vs 一把彩笔」的直觉）。

拆开说，多头相对单个大头的优势有两点：

- **多个独立的注意力分布**。 $H$ 个头产出 $H$ 个**不同**的 $A$ 矩阵，模型能同时维护多套「谁该关注谁」的方案——头 1 盯指代、头 2 盯句法、头 3 盯就近……一个大头只有一个 $A$ ，被迫把这些揉成一个。
- **在不同子空间里看**。每个头先把输入投影到一个 $d_k$ 维子空间再算注意力，相当于「换个角度看数据」。不同头看不同子空间，捕捉的特征更丰富——后面 7.5 节会把不同头明显不同的注意力图画出来，眼见为实。
...
那个 $2$ 是 K 和 V 各一份。注意里头有个 $H$ ——**头越多，缓存越大**。对长上下文（ $L_{\text{seq}}$ 几万）、深层模型（几十层）来说，KV cache 动辄几个 GB，往往比模型权重还吃显存。更要命的是，自回归解码每生成一个 token 都要把整个 KV cache 从显存**读一遍**，所以瓶颈常常不是算力而是**显存带宽**——cache 越大、读得越慢，生成就越卡。

那能不能砍掉 KV cache 里的 $H$ ？这正是 MQA / GQA 的出发点：**让多个 query 头共享 K / V，从而少存几份 K / V**。注意被砍的是 **K / V 的头数**，query 头数 $H$ 不动（不然表达力就掉了）。
...
代价是**表达力下降**：K / V 不再有「多视角」，所有 query 头被迫在同一套 key/value 上找信息，模型质量通常会掉一点。MQA 出现得早（2019），省得狠但伤质量，所以后来有了折中方案。
...
| | query 头数 | K / V 头数 | KV cache | 表达力 | 代表 |
|---|---|---|---|---|---|
...
**Cell 1** 装依赖。本章额外用到 `matplotlib`（画多头注意力热力图）和 `transformers`（7.7 节读 Qwen3 config）：
...
# transformers: 7.7 节用 AutoConfig 读 Qwen3-8B 的 config（num_attention_heads 等）。
...
**Cell 3** 是本章主角。把第 2、3 节的机制和形状变换落成一个 `nn.Module`，**每一步 reshape / transpose 都打印形状**，对照 3.1 / 3.2 节的箭头图看：
...
**Cell 5** 把多个头的注意力矩阵画成热力图，直观感受「不同头关注不同位置」。为了让模式明显，我们手动构造几个头、各自偏好不同的关注方式（实际训练出的头也会自发分化成类似的专门化）：
...
plt.show()
print("→ 4 个头都是下三角（causal），但每个头把注意力分布在不同位置：")
print("  有的头偏好『盯紧自己/对角线』，有的『回看开头』，有的『关注最近一两个』——")
print("  这就是多头的价值：同一序列被多套独立的注意力分布同时审视。")
```

**预期现象**：4 张热力图都是下三角（causal mask 生效），但**亮格分布各不相同**——有的头权重集中在对角线（盯自己）、有的散在第一列（回看开头）、有的偏向次对角线（关注最近）。这就把 4.2 节「多头 = 多个独立注意力分布」从文字变成了可见的图。
...
- 多头**几乎不要钱**：四个投影合计 $4\thinspace d_{\text{model}}^2$ 参数、与 $H$ 无关， $QK^\top$ 的总计算量也和一个大头持平——免费把「一种关注模式」升级成「 $H$ 种」。
