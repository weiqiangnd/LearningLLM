...
> 进阶补充：逻辑上"切片再堆叠"似乎要重排数据，但 PyTorch 实际**只改了 stride**（步长）——底层内存里数据原封不动，只是访问每个元素时按新 stride 跳着取。等到你把它喂给要求 `contiguous` 的算子（或调 `view`），那一刻才会真正发生一次数据拷贝；这也是 `transpose` / `permute` 后常需补一次 `.contiguous()` 的原因。
...
- 参数更新一定要包在 `torch.no_grad()` 里，否则 `a -= lr * a.grad` 这一步本身也会被计入计算图，下一次 `backward` 就会乱套——下一节会展开。
