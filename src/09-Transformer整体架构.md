...
- **Final RMSNorm 容易被忘**：最后一个 block 出来之后、喂给 lm_head 之前，还有一道**末层归一化**。它不属于任何一个 block，是单独加在主干末端的一层，作用是把残差流累加了 36 层之后的数值尺度最后摁一次（Pre-LN 下残差流会一路长大，所以末端必须补一道 Norm 把尺度拉回来——细节见第 5 节）。
...
**三、换装——把 FFN 换成 MoE。** 还记得 FFN 占了一个 block 一大半（约 3/4）的参数吗？改动最大的就在这儿：**MoE（Mixture of Experts，混合专家）** 把每层那**一个** FFN 换成 $E$ 个并列的「专家 FFN」加一个**路由器**，每个 token 只被分给其中 $k$ 个专家去算（比如上百个专家里只激活几个）。这样**总参数量能暴涨十几倍、但单个 token 的计算量几乎不变**——相当于「模型很大，但每次只调用一小部分」。今天最大的那批模型（DeepSeek-V3、Qwen3 的 MoE 版等）走的都是这条路。注意 MoE 动的只是 FFN 子层，**注意力、残差、归一化那套骨架原封不动**——可见本章这套「整体架构」有多通用。
...
| **Final RMSNorm** | 末层归一化，单独加在主干末端、不属于任何 block；Pre-LN 下用来摁住一路长大的残差流尺度 |
...
- **这套骨架既能放大、也能换零件**：放大就是把「宽度 / 层数 / 头数」一起往上推（形状守恒让它退化成调超参，也是 scaling law 的工程前提）；微调有 QK-norm、去 bias、RoPE 长上下文外推等小手术；改动最大的是把 FFN 换成 **MoE**（稀疏专家，总参数暴涨而单 token 算力不变）。但无论怎么改，主干始终是「embedding → $N$ 个形状守恒的 block → final norm → lm_head」这条 Decoder-only 流水线——骨架稳定、零件可换，正是它能撑起整个大模型时代的原因。
