...
实战部分依旧 **全程 CPU、纯 PyTorch**：把第 6-8 章的零件复现出来、拼成一个完整的 mini Decoder-only 语言模型、前向一遍验证逐层形状守恒并数清参数账，再用一个 24 层的深模型亲手验证「Pre-LN 不挑 warmup 也训得动、深层 Post-LN 不加 warmup 几乎学不动」，最后读 Qwen3-8B 的 config，把全景图上的每个数字和真实模型对上。
...
把上面这条流水线画出来，并标出**每个组件的张量怎么变形、各占多少参数**。下面这张全景图就是本章的「技术总览」：左边是宏观数据通路（每格标了输出形状和参数量），右边把**一个 Decoder Block 放大**，展示它内部怎么由 RMSNorm + GQA + SwiGLU 拼装、形状怎么一步步变换；最下面是一笔**参数账**，把 8B 参数拆开，看清楚各个组件占用了多少参数。
...
| 一个 block 合计 | **≈ 193 M** | 42 M + 151 M（FFN 约占 3/4——GQA 缩了注意力，比第 8 章那个 2/3 的下限更高） |
...
**三、大改——把 FFN 换成 MoE。** 还记得 FFN 占了一个 block 一大半（约 3/4）的参数吗？最猛的一刀就砍在这儿：**MoE（Mixture of Experts，混合专家）** 把每层那**一个** FFN 换成 $E$ 个并列的「专家 FFN」加一个**路由器**，每个 token 只被分给其中 $k$ 个专家去算（比如上百个专家里只激活几个）。这样**总参数量能暴涨十几倍、但单个 token 的计算量几乎不变**——相当于「模型很大，但每次只调用一小部分」。今天最大的那批模型（DeepSeek-V3、Qwen3 的 MoE 版等）走的都是这条路。注意 MoE 动的只是 FFN 子层，**注意力、残差、归一化那套骨架原封不动**——可见本章这套「整体架构」有多通用。
...
- **中间路线**：还有人试图取两者之长。**Prefix-LM**（如 UniLM、GLM）让 prompt 部分双向可见、生成部分因果——相当于「前缀当 encoder、续写当 decoder」捏进一个栈。Google 的 **UL2** 则把 MLM / CLM / span corruption 混在一起训。这些都是有意思的探索，但都没能撼动 decoder-only 在通用大模型上的统治地位。
...
总装的第二个设计岔路，藏在每个 block 内部一个看似微不足道的细节里：**归一化（Norm）到底放在子层的前面还是后面？** 第 8 章搭 block 时我们直接用了「放前面」的 Pre-LN，并说「为什么这么放留到本章」。这一节就把这笔账算清楚。
...
这一节**全程 CPU、纯 PyTorch**，路线是：复现第 6-8 章的零件（RMSNorm / RoPE / GQA / SwiGLU） → 拼成一个完整的 Decoder-only mini 模型 → 前向一遍验证逐层形状守恒、数清参数账 → 用一个 24 层的深模型亲手验证 Pre-LN / Post-LN 的训练稳定性差异 → 读真实 Qwen3-8B config 把全景图的数字对上。
