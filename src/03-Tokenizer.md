...
第 2 章我们顺着 `generate()` 一步步走了一遍：logits → temperature → top-k / top-p → softmax → 采样，一个 token 一个 token 地往外输出。但有个问题被我们跳过了——模型输出的是 token **id**（一串整数），喂进去的也是 id，可人类读写的是**文字**。这中间「文字 ↔ id」的翻译工作，就是 **tokenizer（分词器）** 负责的。

先别急着钻进 tokenizer 的细节，我们先来看一下**阶段 2** 的全景。整个**阶段 2**（Transformer 架构精讲，第 3–13 章）做的事，其实就是一块一块地拼出下面这个大语言模型的流水线——从文本进、到下一个 token 出：

![阶段 2 全景：一个 LLM 从文本到下一个 token 的完整流水线](../assets/03/architecture-map.png)

图里每个框的旁边都标了它对应阶段 2 的哪一章，右半边还把第 3–13 章各自的分工逐条列了出来——你可以把它当成整个阶段 2 的「总目录」：读到后面任何一章，都能回这张图看看它落在流水线的哪个位置、和上下游怎么衔接。本章就从最底下那块 **Tokenizer**（文本与 id 之间的翻译工序）讲起。
...
先把第 2 章那条流水线再往前接一截。神经网络只会做数值运算——矩阵乘法、加法、激活函数，它没法直接「读」一个汉字或字母。所以任何文本进模型之前，**必须先变成数字**；模型算完输出的也是数字，要给人看还得**变回文本**。tokenizer 就站在这两端：
...
- **子词（subword）**：介于两者之间——把**高频的片段**当成一个 token。`tokenization` 切成 `token` + `ization`，`机器学习` 切成 `机器` + `学习`。
  ...
  - 这就是**当今几乎所有 LLM 的选择**。代表算法就是本章要讲的 **BPE**。

为什么子词是最优选择？因为「词表大小」和「序列长度」本是一对此消彼长的矛盾——char 把词表压到最小，却让序列爆长；word 把序列压到最短，却让词表爆炸、还带来 OOV。子词的妙处在于**把这两头同时压在可接受的范围内**：
...
> 顺便提一句：token 数直接关系到**钱和速度**。商用 API 按 token 计费，上下文窗口也按 token 算；同样一段话，切出来的 token 越少越省。所以 tokenizer 的「压缩率」（平均每个 token 顶多少个字符）是个实打实的工程指标——第 6 节我们会亲手量一下中英文的差距。
...
BPE 最早是个**数据压缩**算法（1994 年），核心想法非常朴素：**反复把出现得最频繁的相邻一对符号，合并成一个新符号**。2016 年 Sennrich 等人把它搬到 NLP 上做子词切分，从此成了 LLM tokenizer 的事实标准。
...
> 实现上还有个小细节：原始字节里有些是不可打印的控制字符（换行、制表符等），直接当字符串处理会很别扭。GPT-2 提出一个「bytes-to-unicode」的小技巧——把这 256 个字节一一映射到 256 个**可打印**的 Unicode 字符上（比如空格字节映射成 `Ġ`）。所以你在看 BBPE 切出来的 token 时，常会看到 `Ġthe`、`ä¸Ń`（就是上面那个「中」的三个字节映射后的样子）这种「乱码」——那不是 bug，是字节被映射后的可打印形式。这套 256 个字节符号就是词表的基础字母表（base alphabet）。

### 3.3 GPT-2、Qwen 用的就是 BBPE

byte-level BPE 由 GPT-2 带动流行，现在是主流：**GPT 系列、Llama（3 代起）、Qwen、Mistral、DeepSeek** 等的 tokenizer 都是 byte-level BPE（细节各有微调，但「先转字节、256 起步、永不 OOV」这套骨架一致；Llama 2 及更早还是字符级 SentencePiece BPE，靠字节回退兜底，第 5 节的表格里能看到这个代际差）。第 6 节我们加载 Qwen3 的 tokenizer，就能看到它对 emoji、生僻字也能稳稳编码、完美还原。

> 这里再做一个补充：同样叫「字节级」，其实分两种。**BBPE 是带合并的**——base 单位是字节，但照样跑 BPE 把高频字节组合并成更长的子词，所以它**本质仍是子词方法**（本章重点讲的，GPT / Qwen 在用）；而**纯字节级是不合并的**——每个字节直接就是一个 token、词表恒为 256（如 ByT5、Meta 的 BLT，多在研究阶段）。一句话：一个是「在字节上做 BPE」，一个是「干脆每字节一 token」，别弄混。
...
- **但词表越大** → 模型的 **embedding 矩阵**和**输出层**（lm_head）都变大。这两个矩阵的参数量都是 `vocab_size × hidden_size`（一个把 id 变向量、一个把向量变回词表分数，形状互为转置），词表翻倍，这部分参数也翻倍——对小模型来说，词表参数能占总参数相当一块。而且词表太大，很多 token 在训练中出现次数太少，**学不充分**。

所以词表大小是个权衡取舍问题。早期模型（GPT-2）用 50257，近年模型普遍往大了走以更好支持多语言：Llama 3 用约 128k，Qwen3 约 151k（第 2 章 `model.config` 里那个 `vocab_size=151936` 就是它）。多语言模型词表更大很合理——要同时高效编码中、英、日、韩、阿拉伯语等，自然需要更多 token 名额。
...
| `<\|im_start\|>` / `<\|im_end\|>` | 标记一条对话消息的开始 / 结束（ChatML 风格，第 2 章见过） |
...
这也解释了第 2 章那个细节：为什么对话场景要把 `<|im_end|>` 显式加进 `eos_token_id`——因为它就是个特殊 token，模型学会了「该停的时候吐这个 token」，`generate()` 采到它对应的 id 就停。
...
| GPT-2 | byte-level BPE | 50,257 | 最早采用 byte-level BPE 的模型之一；英文为主，中文 / 多语言效率低 |
...
# 注意口径：tokenizer.vocab_size 只统计 BPE 学出来的「基础词表」，不含追加的
# 特殊 token，所以比第 2 章 model.config 里的 vocab_size=151936 略小——config
# 里那个是 embedding 矩阵的行数（基础词表 + 特殊 token，再多留一点余量）。
print("基础词表 vocab_size =", tokenizer.vocab_size)
print("加上特殊 token 后 len(tokenizer) =", len(tokenizer))
...
- 开头打印的两个词表数字不相等是**口径**问题：`tokenizer.vocab_size` 只数 BPE 基础词表，`len(tokenizer)` 把特殊 token 也数进去；第 2 章 config 里的 `vocab_size=151936` 是 embedding 矩阵的行数，在后者之上又多留了一点余量。三个数字逐级略增，都正常。
- 英文里像 `Tokenization` 这种较长的词通常被切成多个子词（如 `Token` + `ization` 这类）；常见短词（`is`、`fun`）整体一个 token。
- 中文一般 1–2 个字一个 token（Qwen 中文语料充足，常见词组被合并得不错）。
- emoji `🤖` 和「北京烤鸭」也能稳稳切出来、`decode` 完美还原——这就是 byte-level BPE 永不 OOV 的体现。值得注意的是，带前导空格的 ` 北` 和裸 `北` 是两串不同的字节：裸 `北` 在中文里高频、词表里有专属 token，三个字节整体并成 1 个；而 ` 北`（空格 + 北）这个整体没合并成专属 token，只能拆成现成碎片，于是切成 2 个 byte token；`京`/`烤`/`鸭` 没有前导空格、是高频的「裸汉字」，各自并成 1 个 token。
- 你会注意到英文 vs 中文「每个 token 覆盖几个字符」差别明显：英文一个 token 常顶好几个字符，中文往往 1–2 个字。这点 6.5 节会专门量化。

### 6.3 纯 Python 手写 BPE 训练

**Cell 3** 用几十行纯 Python 把 2.1 节那个 `hug / pug` 例子的 BPE 训练**完整跑一遍**，打印出每一轮合并了哪对、语料变成什么样——和文档里手推的结果逐字对上：
...
手写版把原理讲清了，但 byte-level、特殊 token、高效实现这些工程环节还没补上。**Cell 5** 用 Hugging Face 的 `tokenizers` 库——和 GPT-2 / Qwen 同款的工业实现——从零训一个 **byte-level BPE**。先造一小段语料写到文件：
...
> 注：`𠮷` 这种字符落到某些终端字体里可能显示成方框（乱码），那是**字体没有这个字形**，不影响 tokenizer 的字节处理——`decode` 照样能精确还原。
...
到这里，第 2 章里那个一直没解释的「文本怎么变成 `input_ids`」终于补齐了。
