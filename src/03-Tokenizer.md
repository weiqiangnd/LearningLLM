...
第 2 章我们顺着 `generate()` 一步步走了一遍：logits → temperature → top-k / top-p → softmax → 采样，一个 token 一个 token 地往外蹦。但有个问题被我们跳过了——模型吐出来的是 token **id**（一串整数），喂进去的也是 id，可人类读写的是**文字**。这中间「文字 ↔ id」的翻译活儿，就是 **tokenizer（分词器）** 干的。
...
先把第 2 章那条流水线再往前接一截。神经网络只会做数值运算——矩阵乘法、加法、激活函数，它没法直接「读」一个汉字或字母。所以任何文本进模型之前，**必须先变成数字**；模型算完吐出来的也是数字，要给人看还得**变回文本**。tokenizer 就站在这两端：
...
byte-level BPE 由 GPT-2 发扬光大，现在是主流：**GPT 系列、Llama（3 代起）、Qwen、Mistral、DeepSeek** 等的 tokenizer 都是 byte-level BPE（细节各有微调，但「先转字节、256 起步、永不 OOV」这套骨架一致；Llama 2 及更早还是字符级 SentencePiece BPE，靠字节回退兜底，第 5 节的表格里能看到这个代际差）。第 6 节我们加载 Qwen3 的 tokenizer，就能看到它对 emoji、生僻字也能稳稳编码、完美还原。
...
所以词表大小是个甜点区问题。早期模型（GPT-2）用 50257，近年模型普遍往大了走以更好支持多语言：Llama 3 用约 128k，Qwen3 约 151k（第 2 章 `model.config` 里那个 `vocab_size=151936` 就是它）。多语言模型词表更大很合理——要同时高效编码中、英、日、韩、阿拉伯语等，自然需要更多 token 名额。
