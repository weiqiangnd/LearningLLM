...
**那为什么能降方差？** 期望不变，但方差 $\text{Var}[\thinspace X - b \thinspace]$ 取决于 $b$ 与 $X$ 的相关程度——选得好的 $b$ （如接近 $\mathbb{E}[\thinspace G_t \mid s_t \thinspace] = V^\pi(s_t)$ ）能把"与状态强相关、与动作选择无关"的那部分波动从 $G_t$ 里扣掉，剩下的就是更"纯净"的关于动作好坏的信号。可以证明在所有只依赖状态的 baseline 里，最优选择正是 $V^\pi(s_t)$ ——这就是 actor-critic 系训一个值网络 $V_\phi$ 当 baseline 的来历。
