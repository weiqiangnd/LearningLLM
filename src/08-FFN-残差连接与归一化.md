...
- **FFN 是什么、为什么需要它？** 注意力本质上是对 value 的加权平均（一种线性的信息混合），缺的是「逐 token 的非线性深加工」。FFN 正是来补这一课——它是 Transformer 里参数量最大的一块。
- **残差连接为什么是深层网络的命根子？** 几十层叠起来，梯度一路反传很容易消失或爆炸。残差给梯度修了一条「高速公路」，让信号直达底层——没有它，深层 Transformer 根本训不起来。
...
实战部分依旧 **全程 CPU、纯 PyTorch**：从零实现 FFN、用一个深层堆叠实证残差对梯度的救命作用、手写 LayerNorm / RMSNorm 并和官方对齐、搭出 SwiGLU 并算清它的参数量、最后把所有零件拼成一个完整的 Transformer block，并读 Qwen3-8B config 看真实模型的 FFN 宽度与归一化配置。
...
- [八、实战：从零搭出 Transformer block 的后半边](#八实战从零搭出-transformer-block-的后半边)
  ...
  - [8.3 残差连接：实证梯度高速公路](#83-残差连接实证梯度高速公路)
...
光把这两块前后接起来还不够。真实模型要把这样的 block 叠**几十层**（Qwen3-8B 是 36 层），而「深」会带来两个老大难问题：梯度反传时容易消失 / 爆炸、各层激活值的数值尺度容易飘。**残差连接**和**归一化**就是来治这两个病的——它们像脚手架一样裹在每个子层外面，让几十层能稳稳地训起来。
...
下面这张图把一个完整 block 的数据流画了出来——注意两条「绕过子层直接相加」的残差旁路，它们是本章第 3 节的主角：
...
中间那个 $d_{\text{ff}}$ 通常取 $d_{\text{model}}$ 的 **4 倍**（原版 Transformer： $d_{\text{model}} = 512$ 、 $d_{\text{ff}} = 2048$ ）。所以 FFN 干的事就是「先把每个 token 的表示**撑大到 4 倍宽**的空间里加工一番，再压回原来的宽度」——像一个先展开、再收拢的「沙漏」反过来（中间胖、两头瘦）。
...
**这正好和注意力形成互补**：注意力负责「跨位置」混合信息（token 之间互相看），FFN 负责「逐位置」深加工（每个 token 关起门来自己算）。一个横着连、一个竖着钻——两者交替，才构成 Transformer 的表达力。
...
而且别小看 FFN 的体量：因为 $d_{\text{ff}} = 4\thinspace d_{\text{model}}$ ，FFN 两个矩阵的参数量是 $2 \times d_{\text{model}} \times 4\thinspace d_{\text{model}} = 8\thinspace d_{\text{model}}^2$ ，而注意力四个投影才 $4\thinspace d_{\text{model}}^2$ （第 7 章第 4 节算过）。**FFN 占了一个 Transformer block 里大约三分之二的参数**。所以「大模型的知识主要存在 FFN 里」这个说法，从参数账上看就很直观。
...
简单粗暴——负的全砍成 0，正的原样保留。它好算、不饱和，但有个小毛病：在 $x < 0$ 的整个区间梯度恒为 0——某个神经元的输入要是长期落在负区，它就再也收不到梯度、参数不再更新，相当于永久失活，这就是所谓的「**死 ReLU**」（dead ReLU）问题；而且在 $x = 0$ 处有个硬拐点、不光滑。
...
训练靠反向传播，要把损失对底层参数的梯度一路从第 $N$ 层乘回第 1 层。根据链式法则，这个梯度是各层雅可比矩阵**连乘**起来的：
...
> 这也是为什么前面 block 公式里写的是 $x + \text{MHA}(\text{Norm}(x))$ 而不是直接 $\text{MHA}(\text{Norm}(x))$ ——那个打头的 $x +$ 就是残差旁路。
...
把很多层叠起来，每一层的输出都会被喂给下一层。训练过程中参数在变，某一层输出的**数值分布**（均值、方差）也跟着不停漂移——这一层这一步还在 $[\thinspace -1, 1\thinspace ]$ 量级，过几步可能就漂到 $[\thinspace -50, 50\thinspace ]$ 。下游层于是永远在适应一个「不断移动的输入分布」，训练既慢又不稳，激活值过大还容易让 softmax / 梯度数值失稳。
...
- $\gamma, \beta \in \mathbb{R}^{d}$ 是**可学习**的逐特征缩放与平移参数， $\odot$ 是逐元素相乘。先把分布标准化掉、再让模型自己学回一个「它想要的」均值方差，是归一化的标准套路。
...
![LayerNorm vs RMSNorm：LayerNorm 减均值再除标准差，RMSNorm 只除 RMS、不减均值、无 β](../assets/08/norm.png)
...
那个 $\sigma(xV)$ 取值在 $[\thinspace 0, 1\thinspace ]$ ，像一道闸门：某一维门接近 1 就让对应的内容通过，接近 0 就掐掉。比起「一个激活一刀切」，门控让网络能**按输入内容自适应地**调节信息流，表达力更强。
...
它长得很像平滑版的 ReLU（ $x$ 大时趋近 $x$ 、 $x$ 很负时趋近 0，但在 0 附近平滑且轻微下探到负值），是 GELU 的近亲。把它装进门控，就得到 SwiGLU 形式的 FFN（写成完整的「升维 + 门控 + 降维」）：
...
- $W_{\text{gate}} \in \mathbb{R}^{d_{\text{model}} \times d_{\text{ff}}}$ ——门路径，过 SiLU 当闸门；
...
所以用 SwiGLU 时，把中间宽度从 $4\thinspace d_{\text{model}}$ 砍到约 $\frac{8}{3}\thinspace d_{\text{model}}$ （即原来 4× 的 **三分之二**），三个矩阵的总参数量就和老式 4× 的两矩阵 FFN 持平——**白赚一个门控、不加参数**。LLaMA 系列就是这么配的（如 LLaMA-2 7B： $d_{\text{model}} = 4096$ 、 $d_{\text{ff}} = 11008 \approx \frac{8}{3} \times 4096$ ）。也正因为换成 SwiGLU 后总参数量和老式 FFN 持平，第 2.3 节说的「FFN 占一个 block 约 2/3 参数」这个比例**依然成立**，并不会因为多了一个矩阵就变。

不过这个 $\frac{8}{3}$ 只是「严格对齐参数」时的取值，各家会按自己的取舍微调，不一定卡死在 2.67×。比如 Qwen3-8B 的 $d_{\text{ff}}$ （config 里叫 `intermediate_size`）就取到 $3\times d_{\text{model}}$ 左右——略大于 $\frac{8}{3}$ ，等于多花一点参数换表达力。第 8.7 节读真实 config 时会把这个比值算给你看。
...
**Q2：残差相加之后，数值不会越加越大吗？**
会有这个倾向（每过一个子层就加一坨东西上去），但**归一化正好接住了它**——下一个子层入口处的 Norm 会把累积起来的尺度重新拉回标准范围。残差负责「让梯度和信息直通」，归一化负责「把数值尺度摁住」，两者搭配，深层才能又稳又训得动。
...
在 LLM 这个场景里，大量实践（LLaMA 起带头）表明 RMSNorm 在质量几乎无损的前提下更省算、更省参数，所以成了主流。但这是「经验结论」而非「数学必然」——RMSNorm 赌的是「减均值这步不重要」，这个赌注在 Transformer 上恰好成立，换个架构未必。
...
这一节**全程 CPU、纯 PyTorch**，路线是：从零实现 FFN → 用深层堆叠实证残差对梯度的救命作用 → 手写 LayerNorm / RMSNorm 并和官方对齐 → 搭出 SwiGLU 并算清参数量 → 把所有零件拼成一个完整的 Transformer block → 读真实 Qwen3 config 看 FFN 宽度与归一化配置。
...
**预期现象**：输出形状和输入一样都是 `[2, 5, 32]`；position-wise 一致性为 `True`——单独喂第 0 个 token 和把它放在整段里算，输出分毫不差，证明 FFN 对每个位置是独立处理的。参数量主项是 $2 \times 32 \times 128$ 。

### 8.3 残差连接：实证梯度高速公路

**Cell 3** 是本章最有「眼见为实」效果的一个实验：搭一个 **40 层**的深层堆叠，分别在「**有残差**」和「**无残差**」两种接法下，做一次反向传播，打印**最底层**收到的梯度大小。无残差时底层梯度会衰减到几乎为 0，有残差时则健康得多：

```python
# ============================================================
# Cell 3: 残差连接实证——深层堆叠下，残差如何救活底层梯度（对应第 3 节）
# ============================================================
...
**预期现象**：SwiGLU 输出形状仍是 `[2, 5, 64]`（形状守恒）；把 $d_{\text{ff}}$ 从 $4 d_{\text{model}} = 256$ 收到约 $\frac{8}{3} d_{\text{model}} \approx 168$ 后，三矩阵 SwiGLU 的参数量和两矩阵老式 FFN 落在同一量级——印证第 5.2 节「白赚一个门控、不加参数」的账（FFN 那侧带 bias，所以不会分毫不差，但主项对齐）。
...
print(f"  d_ff 取到约 {d_ff / d_model:.1f}×d_model，略大于严格对齐的 8/3——多花点参数换表达力。")
```
...
- **残差连接** $x + \text{Sublayer}(x)$ 给梯度修了一条**高速公路**：反传时那个 $+1$ 让梯度直通底层，避免深层的「连乘衰减」；同时让每层默认从恒等映射起步，「加深」在最差情况下也不变糟。实战里 40 层堆叠下，有 / 无残差的底层梯度差出好多个数量级。
...
- 实战我们从零搭了 FFN、用深层堆叠实证了残差的救命作用、手写 LayerNorm / RMSNorm 并和官方对齐、实现了 SwiGLU 并算清参数账，最后把所有零件拼成一个 **Pre-LN + RMSNorm + SwiGLU** 的完整 Transformer block，并读 Qwen3-8B config 看到真实的 SwiGLU + RMSNorm 配置——理论与工业实现在这里闭环。
