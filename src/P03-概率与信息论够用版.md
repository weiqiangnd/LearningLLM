...
LLM 训练里反复出现的几个数学符号——softmax、 $\log p(x)$ 、熵 $H$ 、交叉熵 $H(p, q)$ 、KL 散度 $D_{\text{KL}}(p \thinspace\Vert\thinspace q)$ ——背后是同一套**概率与信息论**语言。把它讲清楚之后，再回头看：
...
> 想直接跑示例？点这里 [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/P03.ipynb)。
>
...
- 看到论文里 $\mathcal{L} = -\mathbb{E}\_{x \sim p_{\text{data}}} [\thinspace \log p_\theta(x) \thinspace]$ （数据分布下的负对数似然，即训练损失），说不清这跟训练循环里调用的 `loss = ...` 是同一件事
- 看到 PPO / DPO 里 $\beta \cdot D_{\text{KL}}(\pi_\theta \thinspace\Vert\thinspace \pi_{\text{ref}})$ （KL 惩罚项，约束新策略不偏离参考分布），不知道这一项为什么能"防止跑偏"
...
- 但它还**不是概率**——既不在 $[\thinspace 0, 1 \thinspace]$ 内，也不归一
...
2. **$q_b$ 偏一点，交叉熵就抬高一点**（ $0.887 > 0.802$ ）——多出来的 $0.085$ 纳特就是"用 $q_b$ 替代 $p$ 多花的代价"，也就是后面 6.2 要讲的 **KL 散度** $D_{\text{KL}}(p \thinspace\Vert\thinspace q_b)$
...
> **关于 one-hot 的一个常见误解**：one-hot 描述的是**单条样本**的标签结构（某位为 1、其余为 0），并**不要求**"同一前缀下的下一个 token 唯一"。回到 3.3 节的例子——「今天天气真 ___」 出现 10 次，标注分别是 7 个 $A$ 、 2 个 $B$ 、 1 个 $C$ ——**这 10 条样本各自的标签仍然是 one-hot**（7 条 $(1, 0, 0)$ 、 2 条 $(0, 1, 0)$ 、 1 条 $(0, 0, 1)$ ），自然语言里"同前缀对应多种延续"这件事并不破坏单样本的 one-hot 性质。把 10 条样本的 NLL 求和 $-(7 \log q_A + 2 \log q_B + \log q_C)$ ，数值上等于"与经验分布 $\hat p = (0.7, 0.2, 0.1)$ 算的交叉熵 $H(\hat p, q)$ "乘以 10——这是**聚合视角**，此时 $\hat p$ 自身**已不是 one-hot**。两种视角等价：模型在大量 one-hot 单样本上同时最小化 NLL，最优解会逼近经验频率分布——这正是 LLM 在 one-hot 标签下仍能学到软概率分布的原因。
...
$$
D_{\text{KL}}(p \thinspace\Vert\thinspace q) = \sum_{i=1}^K p_i \log \frac{p_i}{q_i}
$$

它衡量"用 $q$ 替代 $p$ 损失了多少信息"。性质：

1. $D_{\text{KL}}(p \thinspace\Vert\thinspace q) \ge 0$ ，等号当且仅当 $p = q$
2. **不对称**： $D_{\text{KL}}(p \thinspace\Vert\thinspace q) \ne D_{\text{KL}}(q \thinspace\Vert\thinspace p)$ ——所以严格说不是"距离"
3. **不满足三角不等式**
...
$$
H(p, q) = -\sum_i p_i \log q_i = -\sum_i p_i \log p_i + \sum_i p_i \log \frac{p_i}{q_i} = H(p) + D_{\text{KL}}(p \thinspace\Vert\thinspace q)
$$

**交叉熵 = 真实分布的熵 + KL 散度**。当真实分布 $p$ 固定时（例如训练数据已给定）， $H(p)$ 是常数，**最小化交叉熵就是最小化 KL 散度**。这把"用 cross-entropy 做训练目标"和"逼近数据分布"这两件事正式等同起来。

这条分解也顺便给出了 $D_{\text{KL}}(p \thinspace\Vert\thinspace q) \ge 0$ 的直觉：交叉熵 $H(p, q)$ 是「**用模型 $q$ 描述真实 $p$ 的样本平均要花多少代价**」，而 $H(p)$ 是「**真实 $p$ 自身不可压缩的最低代价**」——用别的分布替代真实分布，代价只会更高、不会更低，所以多出来的差额 $D_{\text{KL}}(p \thinspace\Vert\thinspace q) = H(p, q) - H(p)$ 非负；只有当 $q$ 完美匹配 $p$ 时这笔差额才归零。回到 5.1 的数值例子也能直接验证：当 $q_a = p$ 时 $H(p, q_a) = H(p) \approx 0.802$ ，KL = 0；偏成 $q_b$ 后 $H(p, q_b) \approx 0.887$ ，多出来的 $0.085$ 纳特就是 $D_{\text{KL}}(p \thinspace\Vert\thinspace q_b)$ 。

### 6.3 不对称性

$D_{\text{KL}}(p \thinspace\Vert\thinspace q)$ 跟 $D_{\text{KL}}(q \thinspace\Vert\thinspace p)$ 在优化时倾向不同：

- **forward KL** $D_{\text{KL}}(p \thinspace\Vert\thinspace q)$ ： $p_i > 0$ 的地方 $q_i$ 不能为 0——否则 $q_i \to 0$ 时 $p_i \log(p_i/q_i) \to +\infty$ ，惩罚极重——**逼 $q$ 覆盖 $p$ 的全部支撑**（mode-covering）
- **reverse KL** $D_{\text{KL}}(q \thinspace\Vert\thinspace p)$ ： $p_i = 0$ 的地方 $q_i$ 必须为 0（zero-forcing）——否则 $p_i \to 0$ 时 $q_i \log(q_i/p_i) \to +\infty$ ，惩罚极重——**逼 $q$ 缩进 $p$ 的支撑里**；当 $q$ 容量不够（如单峰高斯拟合双峰 $p$ ）时退化为只挑一个模式（mode-seeking）

下图把双峰真实分布 $p$ 与单峰模型分布 $q$ 在两种 KL 下分别"长成什么样"画在一起：

![forward KL vs reverse KL](../assets/P03/kl-asymmetry.png)

> **注意一个常见误解**：mode-covering / mode-seeking 这组标签描述的是 **$q$ 容量不够**（被参数化成单峰高斯，没法同时覆盖两个峰）时的退化行为，并不是 KL 本身的本质偏好。如果 $q$ 有能力变双峰，两种 KL 的全局最优解都是 $q = p$ ——双峰都覆盖。reverse KL 真正本质的性质是 **zero-forcing**：哪里 $p_i = 0$ 就强迫 $q_i = 0$ （否则 $q_i \log(q_i/p_i) \to +\infty$ ）。容量受限时这条性质退化成"不愿在 $p$ 低概率区误放概率，干脆躲进一个峰"；容量充足时它只是要求 $\text{supp}(q) \subseteq \text{supp}(p)$ ，对峰数没意见。RLHF 里 $\pi_\theta$ 是大型 Transformer 容量近乎无限，所以 $\beta \cdot D_{\text{KL}}(\pi_\theta \thinspace\Vert\thinspace \pi_{\text{ref}})$ 实际起的作用更像"正则化"——防止 $\pi_\theta$ 在 $\pi_{\text{ref}}$ 从不输出的地方乱放概率（reward hacking），而**不是**真把策略压成单一动作。

LLM 里两种 KL 都见得到：

- **预训练 / SFT** 用 cross-entropy ⇔ forward KL（数据是"真实分布" $p$ ）—— 让模型覆盖训练数据的所有可能延续
- **PPO / DPO 等对齐阶段** 加约束 $\beta \cdot D_{\text{KL}}(\pi_\theta \thinspace\Vert\thinspace \pi_{\text{ref}})$ —— 这是 reverse KL（ $\pi_\theta$ 当 "q"、 $\pi_{\text{ref}}$ 当 "p"），逼新策略不要离参考模型太远，防止 reward hacking
...
—— 同一对 $(p, q)$ ， $D_{\text{KL}}(p \thinspace\Vert\thinspace q) \ne D_{\text{KL}}(q \thinspace\Vert\thinspace p)$ 。
...
3. **KL 散度是不对称的**——读 RL / 对齐论文时永远先看清楚他写的是 $D_{\text{KL}}(\pi \thinspace\Vert\thinspace \pi_{\text{ref}})$ 还是反过来
...
- **熵 $H(p)$** 度量分布的不确定性；交叉熵 $H(p, q) = H(p) + D_{\text{KL}}(p \thinspace\Vert\thinspace q)$
