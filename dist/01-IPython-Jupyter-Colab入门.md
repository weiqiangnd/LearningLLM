...
> 想直接跑示例？点这里 [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/01.ipynb)。
>
...
- [二、IPython（Interactive Python）](#二ipythoninteractive-python)
  - [2.1 是什么](#21-是什么)
  - [2.2 核心增强特性](#22-核心增强特性)
  - [2.3 Magic 命令的两种形式](#23-magic-命令的两种形式)
- [三、Jupyter Notebook](#三jupyter-notebook)
  - [3.1 是什么](#31-是什么)
  - [3.2 架构](#32-架构)
  - [3.3 Notebook 的结构](#33-notebook-的结构)
  - [3.4 常用快捷键](#34-常用快捷键)
- [四、Google Colab](#四google-colab)
  - [4.1 是什么](#41-是什么)
  - [4.2 核心优势](#42-核心优势)
  - [4.3 局限](#43-局限)
  - [4.4 基本使用步骤](#44-基本使用步骤)
  - [4.5 在 VS Code 中使用 Colab](#45-在-vs-code-中使用-colab)
- [五、大模型选型：Colab 上跑什么模型最合适](#五大模型选型colab-上跑什么模型最合适)
  - [5.1 Colab 显存档位](#51-colab-显存档位)
  - [5.2 推荐模型（按效果由强到弱）](#52-推荐模型按效果由强到弱)
  - [5.3 综合对比](#53-综合对比)
  - [5.4 关于 4-bit 量化](#54-关于-4-bit-量化)
  - [5.5 Colab Pro 用户的配卡策略](#55-colab-pro-用户的配卡策略)
- [六、实战示例：在 Colab 中加载并运行 Qwen3-8B](#六实战示例在-colab-中加载并运行-qwen3-8b)
  - [6.1 准备工作](#61-准备工作)
  - [6.2 完整代码（含详细注释）](#62-完整代码含详细注释)
  - [6.3 预期输出](#63-预期输出)
  - [6.4 关键概念回顾](#64-关键概念回顾)
- [七、本章小结](#七本章小结)
...
Jupyter 的快捷键分两种模式：**编辑模式**（cell 内有光标、正常打字）和**命令模式**（按 `Esc` 退出编辑、整个 cell 被选中，此时按键就是操作命令）。下面是 Jupyter Notebook / JupyterLab 的原生键位：

| 快捷键 | 模式 | 作用 |
|--------|------|------|
| `Shift + Enter` | 任意 | 运行当前 cell 并跳到下一格 |
| `Ctrl + Enter` | 任意 | 运行当前 cell，停留在原处 |
| `B` / `A` | 命令模式 | 在下方 / 上方新建 cell |
| `D D`（连按两次 D） | 命令模式 | 删除当前 cell |
| `M` | 命令模式 | 切换为 Markdown cell |
| `Y` | 命令模式 | 切换为 Code cell |

> 注意：**Colab 的键位和原生 Jupyter 不一样**——它把命令模式单键统一改成了 `Ctrl + M` 前缀的组合键：`Ctrl + M B` 新建、`Ctrl + M D` 删除、`Ctrl + M M` / `Ctrl + M Y` 切换 cell 类型。下一节进入 Colab 后用的就是这一套。
...
| Pro+ | A100 | 40 GB | 同 Pro 的 GPU，但算力配额更高、可后台运行、会话更长 |
...
- **显存**：BF16 约 16GB（T4 装不下，需量化），4-bit 约 6GB
...
3. 计算缩放系数 `scale = absmax / 7`（4-bit 有符号整数范围是 −8 到 7）
...
$$
W = [\thinspace 0.42, {-1.23}, 0.78, 1.40, {-0.34}, 1.05, {-0.91}, 0.12 \thinspace]
$$
...
#### 为什么是 4-bit：精度和体积的平衡点

| 位数 | 显存压缩比 | 质量损失（典型 7-8B 模型） | 评价 |
|------|------------|----------------------------|------|
| 8-bit | 1/2 | < 0.5%（近乎无损） | 显存优势不够大 |
| **4-bit** | **1/4** | **1–2%** | **平衡点：体积大幅缩小、损失可接受** |
| 3-bit | 3/16 | 5–10% | 损失明显，仅特定校准方案勉强可用 |
| 2-bit | 1/8 | > 15% | 损失太大，研究阶段 |
...
- **双量化（double quantization）**：每个块都有自己的 `scale` 系数，而 `scale` 本身也是浮点数。把这些 `scale` 再做一次 8-bit 量化，每个权重再省 ~0.4 比特。8B 模型大概再省 0.4 GB。

> 还有 **GPTQ / AWQ** 等"校准量化"方案——量化前用一小批样本数据校准每层的量化参数，质量损失通常比 NF4 再低 0.5%。本章用 NF4 是因为它"开箱即用"、不需要准备校准数据。
...
- **显存与显存带宽**：模型权重和 KV cache 都存在 GPU 显存（VRAM）里，每次 matmul 都要先把权重从显存搬到 SM 内的寄存器/SRAM 才能参与计算，"搬得多快"由显存带宽决定。数据中心卡（A100/H100）用的是紧贴芯片封装的 **HBM（High Bandwidth Memory，高带宽显存）**；T4 这类老卡用的是普通 **GDDR6**（320 GB/s，约为 A100 HBM2e 1.5 TB/s 的 1/5），但"权重必须先搬运再计算"这一点是一样的。
- **memory-bound（内存带宽瓶颈）**：瓶颈在"从显存把数据搬到计算单元"上——计算单元大多时间空转等数据。
...
##### 用 T4 + Qwen3-8B 估算一下单 token 耗时

T4 的两项关键指标：显存（GDDR6）带宽约 320 GB/s，FP16 Tensor Core 算力约 65 TFLOPS。每生成一个 token，所需资源的耗时下限：

| 资源 | 公式 | 耗时下限 |
|------|------|----------|
| 从显存搬一遍 4-bit 权重（≈ 5.5 GB） | $5.5\text{ GB} / 320\text{ GB/s}$ | **~17 ms** |
...
### 5.5 Colab Pro 用户的配卡策略

Colab Pro 提供 ~100 计算单元/月，可访问 L4 / A100 / v5e / v6e，并支持后台执行与更长会话。但**不同 GPU 的算力消耗差异很大**，需要按场景规划。
...
A100 消耗最快、T4 最省——8 小时 vs 50 小时差 6 倍。学习场景最忌讳被算力卡住中断，所以要分场景配卡。
...
| 日常学习主力（推理、对比） | **L4** | 24GB 显存够 7B BF16 + 13B 4-bit；compute 受限场景比 T4 明显快，decode 受显存带宽限制提升有限 |
...
阶段 1：基础概念
    ...
    → 跑教程、上手 tokenizer、采样策略、思考模式开关，省单元
...
**Cell 2** 用 4-bit NF4 + 双量化加载模型与分词器：
...
# Qwen3 对话场景的终止符是 <|im_end|>（标记一轮消息说完了）。它在 tokenizer_config.json
# 里被静态配置成 eos_token——所以 Qwen3 的 tokenizer.eos_token_id 拿到的就是 <|im_end|>；
# 另有一个通用文本结束符 <|endoftext|>，仓库的 generation_config.json 默认把两者都列为终止 token。
# 这里把两个 id 都显式查出来传给 generate()，让"在哪停"在代码里看得见；
# 注意：显式传 eos_token_id 会整体覆盖 generation_config 的默认列表——要传就两个都带上
...
with torch.no_grad():
    outputs = model.generate(
        ...
        **inputs,
        ...
        top_p=0.8,
        top_k=20,
        # eos_token_id 标记生成结束的 token，采到列表中任意一个就停止
        # <|im_end|>（对话轮结束）+ <|endoftext|>（通用结束），与 generation_config 默认一致
...
| **终止符（eos）** | Qwen3 把对话终止符 `<\|im_end\|>` 配置为 eos_token；显式传 `eos_token_id` 列表会覆盖默认值，记得连 `<\|endoftext\|>` 一起带上 |
