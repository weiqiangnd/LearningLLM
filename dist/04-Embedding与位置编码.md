...
**Cell 9** 实现 RoPE（第 6 节），并**实证核心性质**：旋转后 query / key 的点积只依赖相对位置 $n - m$ ：
...
**预期现象**：三组**相对距离都为 1** 的 `(m,n)` 算出的分数几乎完全相等（小数点后多位一致）——印证「分数只依赖相对位置 $n - m$ 」；而相对距离从 1 变到 2、5 时分数明显不同。这就是第 6.2 节那条性质的代码实证。
