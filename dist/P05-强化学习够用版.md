...
- $G_t$ 小但为正 → 仍然是**推大**，只是推得弱——注意这一点：只要回报是正的，朴素版公式对采到的每个动作都在加码（CartPole 这种 $G_t$ 恒正的环境里尤其明显），"比平均差的动作被压下去"要等第 6 节引入 baseline 之后才字面成立
...
最简单的 baseline 是把 batch 里所有 $G_t$ 减去它们的均值——一行代码，立竿见影；实战里通常顺手再除以标准差做归一化（注意：除以 std 已经超出 baseline 理论的范围，是对梯度的整体缩放，严格说让估计**有偏**，但能把更新步幅稳定到同一量级，实践中几乎总是利大于弊）：
