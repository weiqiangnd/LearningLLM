...
第 1 章我们让 Qwen3-8B 跑通了一次对话——加载模型、构造 messages、调用 `generate()`、decode 输出。但 `generate()` 里那一长串参数（`temperature` / `top_p` / `top_k` / `do_sample` / `repetition_penalty` ……）当时只是按 Qwen3 官方推荐值贴了一组，没解释。
...
> 想直接跑示例？点这里 [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/02.ipynb)。
>
> **硬件门槛**：Colab 免费版 T4（15 GB）✅。本章沿用第 1 章的 4-bit 量化 Qwen3-8B，T4 即可跑通全部 6 组对比实验。打开 ipynb 前请先在 Colab 菜单 **Runtime → Change runtime type** 里切到 GPU。

> 〔预备知识〕本章首次用到 softmax（把一串 logits 压成概率分布）、temperature 对分布尖锐 / 扁平的影响，以及由此衍生的 top-k / top-p 采样——这些都建立在「概率分布」这套语言上。若对 softmax、log-likelihood、熵 / 交叉熵 / KL 这些概念不熟悉，建议先读 P03。
...
- [三、采样策略：从分布里挑一个 token](#三采样策略从分布里挑一个-token)
  ...
  - [3.4 Top-p（nucleus）采样](#34-top-pnucleus采样)
  - [3.5 三者的组合顺序](#35-三者的组合顺序)
- [四、`generate()` 参数全景](#四generate-参数全景)
- [五、实战：参数对比实验](#五实战参数对比实验)
  - [5.1 准备工作：GPU 自检 + 装依赖 + 加载模型 + chat() 辅助函数](#51-准备工作gpu-自检--装依赖--加载模型--chat-辅助函数)
  - [5.2 实验 1：贪心 vs 采样](#52-实验-1贪心-vs-采样)
  - [5.3 实验 2：temperature 高低对比](#53-实验-2temperature-高低对比)
  - [5.4 实验 3：top\_p 严格 vs 宽松](#54-实验-3top_p-严格-vs-宽松)
  - [5.5 实验 4：repetition\_penalty 抑制重复](#55-实验-4repetition_penalty-抑制重复)
  - [5.6 实验 5：思考模式 vs 非思考模式](#56-实验-5思考模式-vs-非思考模式)
  - [5.7 实验 6：复现性与随机种子](#57-实验-6复现性与随机种子)
- [六、关键概念回顾](#六关键概念回顾)
- [七、本章小结](#七本章小结)
...
第 1 章里我们用一行 `AutoModelForCausalLM.from_pretrained("Qwen/Qwen3-8B", ...)` 把模型加载进了显存，但这行代码内部到底干了什么、那 16 GB（fp16 体积；本章 4-bit 加载后落进显存的只有约 5.5 GB）权重存在哪？本节先把"加载"摊开讲，再接上"生成"的流水线，串起从硬盘到下一个 token 的完整链路。
...
└── models--Qwen--Qwen3-8B/
    ...
    └── snapshots/
        └── <commit-sha>/            # 用软链指向 blobs 里的真实文件
            ...
            ├── model-00001-of-00005.safetensors  # 模型权重，分片存储（每片 3–4 GB）
...
- **`model-XXXXX-of-NNNNN.safetensors`**：模型权重本体，切成多片存储——Qwen3-8B 是 5 片、每片 3–4 GB（`save_pretrained` 默认的单片上限是 5 GB，具体切法由模型作者保存时决定）。Qwen3-8B 在 bf16/fp16 下总大小约 16 GB（80 亿参数 × 2 字节）。**safetensors** 是 Hugging Face 推出的纯数据格式（header 是 JSON、body 是连续的 tensor 字节），相比早期 PyTorch 的 `pytorch_model.bin`（pickle 序列化）安全得多——pickle 反序列化时能执行任意代码，下载来路不明的 `.bin` 有真实风险。safetensors 还支持 mmap 零拷贝加载，速度通常更快。
- **`model.safetensors.index.json`**：分片索引，记录每个权重张量（如 `model.layers.0.self_attn.q_proj.weight`）实际存在哪个分片。加载时按它逐张量定位。
- **`tokenizer.json` + `tokenizer_config.json`**：tokenizer 的全部信息——词表、BPE（Byte Pair Encoding，字节对编码）合并规则、特殊 token、chat template（一段 Jinja 模板，定义如何把 messages 拼成训练时见过的格式）。BPE 的原理与作用第 3 章会专门讲。
- **`generation_config.json`**：模型作者写好的默认生成参数。Qwen3-8B 这份文件里是 `do_sample=true`、`temperature=0.6`、`top_p=0.95`、`top_k=20`——注意这是**思考模式**（Qwen3 的默认模式）的官方推荐值；非思考模式官方另推荐 `temperature=0.7`、`top_p=0.8`，需要自己在 `generate()` 里显式传。`generate()` 没显式指定的参数都从这份文件取。

> 想亲眼看一下磁盘上的样子？第 5 节实战的 **Cell 3** 会列出 cache 里所有文件大小、并打印出 `model.config`——把这一节讲过的几个概念（缓存目录、分片、`config.json` 字段）和磁盘上的真实文件一一对上。
...
`AutoTokenizer.from_pretrained()` 类似，只不过它读的是 `tokenizer.json` + `tokenizer_config.json`，构造出一个 `Qwen2Tokenizer` 实例——Qwen3 沿用 Qwen2 的 tokenizer 实现，所以类名是 Qwen2；底层由 Rust 实现的 tokenizers 库驱动。顺带一提：transformers v4 时代 tokenizer 分 Python 慢速版与 Rust fast 版两套类、`AutoTokenizer` 默认返回 `Qwen2TokenizerFast`，v5 起两套合并为一个类、不再带 Fast 后缀——所以在旧环境里 `print(type(tokenizer))` 看到 `Qwen2TokenizerFast` 也是正常的。
...
- **采样（sampling）**：从概率分布里挑一个 token 出来。第 3 节展开"怎么挑"。
...
2. **分母 $\sum_j \exp(\ell_j)$ ——归一化**。把所有 token 的 $\exp$ 值加起来，作为统一的"总分"。每个 token 的概率就是"它的份额 ÷ 总份额"，自然落在 $[\thinspace 0, 1 \thinspace]$ ，且全部加起来等于 1，满足合法概率分布的定义。

一句话总结：**先用 $\exp$ 把分数变成正的、且拉开差距，再除以总和分掉这块"概率蛋糕"**。

> 一个常用直觉：softmax 可以理解成「**带权抽奖**」——每个 token 拿到 $\exp(\ell_i)$ 张彩票，分母就是所有人的彩票总数，相除得到的就是这张票的中奖概率。logit 高的 token 多领指数级数量的彩票。
...
假设词表只有 4 个 token（玩具情形），logits 是 $\boldsymbol{\ell} = [\thinspace 2.0,\ 1.0,\ 0.5,\ 0.1 \thinspace]$ 。算一下不同 $T$ 下的概率分布（两位小数）：
...
> 这张表对应第 5 节实战的 **Cell 4**——一段不到 10 行的 PyTorch 代码就能打印出来，建议跑一遍直观感受。
...
第 1 章用过 `tokenizer.apply_chat_template(messages, ...)`，但这一步内部到底做了什么？
...
顺手提一句：上面是 `enable_thinking=True`（Qwen3 的默认）时的形态。`enable_thinking=False` 时，模板还会在 `<|im_start|>assistant\n` 后面**替模型补一对空的思考标签** `<think>\n\n</think>`——相当于把"思考"环节预先填成空白，模型看到它就跳过推理、直接写答案。所以跑本章实验时若把 `inputs["input_ids"]` decode 出来看，末尾多出的这对空标签不是 bug，而正是非思考模式的实现方式。
...
| `<think>` / `</think>` | 思考模式（thinking mode）的开闭定界符；`enable_thinking=True` 时模型会先在这对 token 之间输出推理过程，再给最终答复。这是 Qwen3 区别于 Qwen2.5 的核心特性 |
...
Qwen3 在 chat 数据上学到的是"这一轮该说完时就吐 `<|im_end|>`"，因此对话场景下真正的终止符是 `<|im_end|>` 而不是通用的 `<|endoftext|>`。Qwen3 干脆在 `tokenizer_config.json` 里把 `eos_token` 静态配置成了 `<|im_end|>`——所以 `tokenizer.eos_token_id` 拿到的就是它；仓库的 `generation_config.json` 则默认把 `<|im_end|>` 和 `<|endoftext|>` **都**列进了终止 token 列表。第 1 章里我们写过：
...
把两个 token id 都显式查出来传给 `generate()`（这个参数支持 list，采到任意一个就停）。严格说不传也行——`generate()` 会从 `generation_config.json` 取到同样的列表；显式写出来一是让"在哪停"在代码里看得见，二是这个自查动作换到别的模型家族也通用。要注意**显式传 `eos_token_id` 会整体覆盖默认列表**，所以要传就把两个都带上，别只传一个。本章后面所有实验都沿用这个写法。
...
$$
P'_i = \begin{cases}
\dfrac{P_i}{\sum_{j \in \text{TopK}} P_j} & i \in \text{TopK} \cr
0 & \text{otherwise}
\end{cases}
$$
...
原始 logits
   │
   ▼  repetition_penalty（对已出现 token：正除负乘；作用在原始 logits 上、早于 temperature）
惩罚后 logits
   │
   ▼  ÷ temperature
缩放后 logits
...
另外提一句：第 3.3 / 3.4 节的公式是按**概率空间**写的（先归一化、再挑候选），而 transformers 实现里 top-k / top-p 直接在 **logit 空间**把被截掉的候选置成 $-\infty$ 、最后只做一次 softmax。softmax 单调、不改变排序，两种写法选出的候选集和最终概率完全一致，纯属实现效率上的安排。
...
| `eos_token_id` | 循环终止条件 | generation_config 默认 | 采到该 id 就停。可传 list 表示"采到任一一个就停"；显式传会覆盖默认列表 |
...
> 表中「默认值」是 transformers 的**库级默认**。别忘了第 1.1 节讲过的覆盖关系：模型仓库的 `generation_config.json` 会盖掉这层库级默认——Qwen3-8B 那份文件里写着 `do_sample=true`、`temperature=0.6`、`top_p=0.95`、`top_k=20`，所以对本章的模型来说，**不传 `do_sample` 时实际走的是采样、不是贪心**；想要贪心必须显式 `do_sample=False`。本章实验把每个关键参数都显式传一遍，就是为了不被这层覆盖关系干扰。
...
> 单 batch decode 的带宽理论下限约 17 ms/token（详见第 1 章「反量化会拖慢推理速度么」一节）——但 T4 上 4-bit 的 dequant kernel 没和 matmul 融合，**实测会更慢**，约 10–15 tok/s（即几十毫秒一个 token，详见第 5.6 节的耐心提示）。每个实验跑 2–3 次、200–300 个新 token，单 cell 耗时约几十秒到一两分钟。

### 5.1 准备工作：GPU 自检 + 装依赖 + 加载模型 + chat() 辅助函数
...
# 4-bit 量化配置（原理与手算示例见第 1 章「关于 4-bit 量化」一节）
...
# 提前查好两个终止符的 token id：<|im_end|>（对话轮结束；Qwen3 的 eos_token
# 在 tokenizer_config.json 里就被配置成它）和 <|endoftext|>（通用文本结束）。
# 显式传 eos_token_id 会覆盖 generation_config 的默认终止列表，所以两个都带上
im_end_id = tokenizer.convert_tokens_to_ids("<|im_end|>")
endoftext_id = tokenizer.convert_tokens_to_ids("<|endoftext|>")
...
print("\n【多项式采样 do_sample=True, temperature=1.0, top_p=0.95, top_k=20】")
for i in range(3):
    torch.manual_seed(i)  # 每次用不同 seed，让采样路径不同
    print(f"  Run {i+1}: {chat(prompt, do_sample=True, temperature=1.0, top_p=0.95, top_k=20)}")
```
...
# Prompt 要选"头部分布天然就平坦"的任务——比如"取名字"，几十个候选
# 概率都差不多；而像"推荐一种宠物"这种，仓鼠/金鱼会瓜分大部分概率，
# 即使温度拉到 1.5、top_p 放到 0.95，也几乎总是挑到同一个，看不出差异
prompt = "给一只虚拟小狗取一个可爱的名字，并简单说明寓意。"
...
for p in [0.3, 0.95]:
    ...
    for i in range(2):
        torch.manual_seed(i)
        print(f"--- Run {i+1} ---")
        print(chat(prompt, do_sample=True, temperature=1.0, top_p=p, top_k=0))
```

**预期现象**：

- `top_p=0.3` 两次输出取的名字高度集中在极少数高频候选上，重复率高、风格趋同
- `top_p=0.95` 输出的名字更五花八门，长尾候选有机会被采到，两次结果差异明显

**结论**：top_p 控制"愿意考虑多少长尾候选"。需要严谨/可控就调小，需要多样性就调大。

### 5.5 实验 4：repetition_penalty 抑制重复

`repetition_penalty` 的做法是：对**已经出现过的 token**，把它的 logit 按一个 $> 1$ 的系数往更小推——**正 logit 除以系数、负 logit 乘以系数**（两个分支都让这个 logit 变小；默认 1.0 = 不惩罚）。注意不能一律「除以系数」：负 logit 除以 $> 1$ 反而会变大（例如 $-2.0 / 1.2 = -1.67$ ，比 $-2.0$ 还大），把概率推高，正好抑制反了——所以 HuggingFace 对负 logit 改用乘法。系数越大，已出现 token 在后面被再次采到的概率越低。

位置上提一句：repetition_penalty 作用在**原始 logits** 上、排在 temperature **之前**（HF 里它属于 `LogitsProcessor`，早于做缩放/截断的 temperature/top-k/top-p），就是 3.5 节那张流程图最前面那一步。
...
### 5.6 实验 5：思考模式 vs 非思考模式

> ⚠️ **耐心提示**：思考模式 `max_new_tokens=2048`，T4 上 4-bit Qwen3-8B 速度约 10–15 tok/s，**单次推理需要 2–4 分钟**；本章 `chat()` 用的是非流式生成，期间 cell 不会有任何中间输出——看着像 hang，其实在算，等就行了。想看 GPU 是否还在工作，可在另一个 cell 跑 `!nvidia-smi` 看占用。
...
- 非思考模式通常能直接给出正确答案（Qwen3-8B 这种规模已经能 hold 住二元一次方程组），或简短列一下方程但跳步，过程较短
- 思考模式先输出一段 `<think>...</think>` 推理（设变量 → 列方程 → 解方程 → 验算），过程明显更详细、更"啰嗦"，最后给出 24/7 ≈ 3.43 元这个非整数解

> 注：这道题的真实答案——3a + 2b = 14、5a + b = 19，解得 $a = 24/7 \approx 3.43$ 元， $b = 13/7 \approx 1.86$ 元。这种基础线性方程组对 8B 量级模型不算难，两种模式都大概率算对；想观察思考模式带来的"准确率"提升，需要换更难的题（如奥数 / AIME 类）。本实验主要让你直观看到**两种模式输出形态上的差异**——思考模式会显式列出推理链条。
...
| **logits** | 模型对词表中每个候选 token 的"原始评分"；前向输出形状 `[B, L, V]`，采样只用最后位置的 `[V]` 切片 | 模型前向输出 |
...
| **repetition_penalty** | 把已出现 token 的正 logit 除以、负 logit 乘以一个 $> 1$ 的系数 | logits 调整阶段 |
...
- **EOS** 是停止信号；Qwen3 把对话终止符 `<|im_end|>` 配置为 eos_token，显式传 `eos_token_id` 会覆盖 generation_config 的默认列表，记得连 `<|endoftext|>` 一起传
