...
> 想直接跑示例？点这里 [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/weiqiangnd/LearningLLM/blob/main/src/P02.ipynb)。
>
> **硬件门槛**：概念章，CPU 即可✅。本章用一个 2 层 MLP 在 1000 个二维点上做二分类，参数总数不超过 200，CPU 训练一轮不到 1 秒。

## 目录

- [一、MLP：最简单的可训练神经网络](#一mlp最简单的可训练神经网络)
- [二、MLP 是什么](#二mlp-是什么)
  - [2.1 数学定义](#21-数学定义)
...
> - **CDF**：Cumulative Distribution Function，累积分布函数。 $\Phi(x) = P(X \le x)$ ，其中 $X \sim \mathcal{N}(0, 1)$ 是标准正态变量； $\Phi(x)$ 把 $x$ 映射到 $[\thinspace 0, 1 \thinspace]$ ，于是 GELU 可以理解为「ReLU 的平滑版本」——用 $\Phi(x)$ 这个软门控代替 ReLU 的硬阈值 $\mathbb{1}[\thinspace x > 0 \thinspace]$ 。
...
- **负半轴**：ReLU 直接归零（这就是「死亡 ReLU」的源头：一旦输入持续为负，梯度也是 0，权重永远更新不动）；GELU / SiLU 在 $x \in [\thinspace {-1.5}, 0 \thinspace]$ 范围内留了一个"小坑"——GELU 最低点约 -0.170（在 $x \approx -0.75$ 处），SiLU 最低点约 -0.278（在 $x \approx -1.278$ 处），允许少量负信息和梯度漏过去
...
- **SiLU 只作用在 gate 这一路**：把 gate 通道的值大体压到非负——但留有一个"小坑"，最低约 -0.278（出现在 $z \approx -1.278$ 处，见前面那条橙色虚线）。直观上：大正输入 ≈ 原样放行，强负输入压到接近 0，轻度负输入留一点"软门"漏过去。所以"通过比例"这个词只是大体直觉，不要把它误解成严格的 $[\thinspace 0, 1 \thinspace]$ 门——SwiGLU 里的 gate 上不封顶、下也允许小幅负值
...
$$
W_1 x = [\thinspace 2.0,\thinspace {-1.0},\thinspace 0.5\thinspace], \quad W_2 x = [\thinspace 1.0,\thinspace 3.0,\thinspace {-2.0}\thinspace]
$$

先算 gate—— $\mathrm{SiLU}(z) = z \cdot \sigma(z) = z / (1 + e^{-z})$ ，逐元素套上去：

$$
\mathrm{SiLU}(W_1 x) = \big[\thinspace 2.0 \cdot \sigma(2.0),\thinspace\thinspace {-1.0} \cdot \sigma(-1.0),\thinspace\thinspace 0.5 \cdot \sigma(0.5)\thinspace\big]
$$

$$
= [\thinspace 2.0 \times 0.881,\thinspace\thinspace {-1.0} \times 0.269,\thinspace\thinspace 0.5 \times 0.622\thinspace] \approx [\thinspace 1.76,\thinspace {-0.27},\thinspace 0.31\thinspace]
$$

再按位乘 value：

$$
\mathrm{SwiGLU}(x) \approx [\thinspace 1.76 \times 1.0,\thinspace\thinspace {-0.27} \times 3.0,\thinspace\thinspace 0.31 \times {-2.0}\thinspace] = [\thinspace 1.76,\thinspace {-0.81},\thinspace {-0.62}\thinspace]
$$

注意第二个通道：value 是 3.0（很大），但 gate 落在 SiLU 的负半轴小坑里（ $W_1 x = -1$ ， $\mathrm{SiLU}(-1) \approx -0.27$ ），最终输出被缩到 -0.81——量级压到 value 的约 1/4，方向还反了。这就是"门控"的字面含义：**用一路信号决定另一路每个通道的"放大倍数"**（在 SwiGLU 里这个倍数大体非负、但允许小幅负值，所以严格说是"加权"而非传统意义的 $[\thinspace 0, 1 \thinspace]$ 门）。
...
| 第 2 节里的理论概念 | `nn.Module` 体系里对应的对象 | 形状 / 备注 |
|---|---|---|
| 权重矩阵 $W_l$ | `nn.Linear(in, out).weight` （`nn.Parameter`） | `(out, in)` ，可训练 |
| 偏置向量 $b_l$ | `nn.Linear(in, out).bias` （`nn.Parameter`） | `(out,)` ，可训练 |
| 一次仿射变换 $W_l h_{l-1} + b_l$ | `nn.Linear(in, out)` 整层 | forward 时算 $x W^\top + b$ ，详见第 4.1 节 |
...
下面就把第 2.1 节那个 2 层 MLP（ $f_\theta(x)$ 在 $L=2$ 时的具体形态）按这两条约定写出来——`fc1` / `fc2` 装的正是 $W_1, b_1$ 和 $W_2, b_2$ 这两组参数，`forward` 里的两行代码逐字对应公式 $h = \mathrm{ReLU}(W_1 x + b_1)$ 和 $y = W_2 h + b_2$ ：
...
注意是 $W^\top$ ——**PyTorch 内部把 weight 存成 `(out, in)`**（约定如此，许多深度学习框架都是这一布局：每一行对应一个输出神经元的输入权重）。所以 `linear.weight.shape == (out, in)`，乘的时候转置一下。这条容易记错：调试时 `print([p.shape for p in linear.parameters()])` 一眼就能看清（注意要套一层列表推导——直接 `print(p.shape for p in ...)` 打出来的是一个 generator 对象，什么也看不到）。
...
$$
\mathcal{L} = -\big[\thinspace y \log \sigma(z) + (1-y) \log (1 - \sigma(z))\thinspace\big], \quad \sigma(z) = \frac{1}{1 + e^{-z}}
$$
...
当 logit $z$ 很负时 $\sigma(z) \to 0$ 、 $\log \sigma(z) \to -\infty$ ；很正时 $1 - \sigma(z) \to 0$ 、 $\log(1 - \sigma(z)) \to -\infty$ 。朴素写法在浮点数上挂得比直觉早得多：**正方向 fp32 在 $z \approx +17$ （fp16 约 $+9$ ）时 $\sigma(z)$ 就被舍入成恰好 1.0**，于是 $\log(1 - \sigma(z)) = \log 0 = -\infty$ ——这一侧的杀手不是 exp 上溢，而是"概率被舍入到 1"；负方向则要到 $z < -88$ （fp16 约 $-11$ ， $e^{-z}$ 超过该 dtype 最大可表示数）才会上溢成 `inf` 把 $\sigma(z)$ 压成 0。一旦出现 $\log 0$ ，loss 先变 `inf`，再混进 $0 \times \infty$ 这类组合就成了 `nan`。融合版本改写成数值稳定的等价形式：

$$
\mathcal{L} = \max(z, 0) - z \cdot y + \log(1 + e^{-|z|})
$$

关键是 $e^{-|z|} \le 1$ 不会上溢， $\log(1 + e^{-|z|})$ 始终落在 $[\thinspace 0, \log 2 \thinspace]$ ，再大的 $|z|$ 也不会让中间结果越界。**所以传 raw logit 就行；手动先 `sigmoid` 再用 `BCELoss` 等于自愿退回那个不稳定的版本**。
...
减完之后最大那一项的 $e^{z_j - m} = 1$ ，其余都在 $(0, 1 \thinspace]$ ， $\sum$ 既不会上溢、 $\log$ 也不会拿到 0。拿到所有类别的 $\log p_i$ 之后，loss 只是取第 $c$ 个分量再取负——这正是 PyTorch 把它叫 `log_softmax + NLL`（Negative Log Likelihood，负对数似然）的字面含义。

同样地，`F.cross_entropy(logits, target)` 期望传入 **raw logits**——它内部自己 `log_softmax` 一次。手动 `softmax` 过再传**不会报错**，但 loss 已经不是真正的 NLL：它有一个抬高的下界（模型再笃定也降不到 0）、概率校准失效、接近 one-hot 时梯度被压扁。最阴险的是小任务上准确率往往照常上升、训练曲线看不出异常——这类 bug 靠肉眼盯曲线抓不到，调试时容易卡很久。
...
> 顺带预告：本节出现的几个名字——**softmax / log-softmax / log-sum-exp / cross-entropy / NLL**——下一章 P03 会从**概率与信息论**视角再讲一遍。
...
其中 $g_t = \nabla_\theta \mathcal{L}$ 是第 $t$ 步的梯度， $\beta_1 \in [\thinspace 0, 1)$ 是衰减系数（PyTorch 默认 0.9）， $m_0 = 0$ 。 $m_t$ 是历史梯度的指数加权平均（EMA），方向上比单步 $g_t$ 更平滑——这就是"动量"的直觉：连续几步同向时累加加速，方向震荡时彼此抵消。
...
- **训练后（epoch 200）**：边界开始在两个半月之间**弯曲**，沿着两类的交界处贴合、acc ≈ 0.87——非线性激活让多层有意义的可视化证据。准确率没到 100% 是 `noise=0.2` 的设置让边界附近本就有交叉点，加上 SGD lr=0.1 200 epoch 在这个 2-16-1 的迷你 MLP 上大约就在 87% 左右收敛（换 AdamW 或加宽隐藏维度能再涨到 95%+，留给读者自行实验）
...
并对比一个**故意去掉 ReLU** 的退化模型（`y = fc2(fc1(x))`）：会看到无论训多久，决策边界都只是一条直线（"两个线性层叠起来等价于一个线性层"在数据上的实证）。注意**重点不是准确率**——make_moons 上这个线性模型照样能到 acc ≈ 0.86，和带 ReLU 的差不多；关键是它的边界永远是直线，没法贴合两个半月的弯曲交界。
...
| loss 突然变 NaN / inf | lr 太大、loss 内部出现 log(0) | 减小 lr；改用数值稳定的 loss API——把 raw logits 传给 `binary_cross_entropy_with_logits`。注意 `binary_cross_entropy(sigmoid(x))` 不一定表现为 NaN（PyTorch 内部把 log 钳位在 −100），更多时候是 logit 一大梯度就饱和失真、训练悄悄变坏 |
