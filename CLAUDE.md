...
- **表达尽量平实，减少口语化用词**——可以像跟同事白板边讲边推演那样自然好读，但别靠堆口语、俏皮、夸张来"显得亲切"，也别走到另一头写成教科书式的"陈述句堆砌"。下面几条把握分寸：
  ...
  - **俏皮表达有个「慎用清单」**——它们偶尔点一句有味道，但草稿里有反复堆砌、过密的倾向，审阅时几乎都会被换成平实说法，所以下笔时就克制：
    ...
    - **网络梗、夸张绝对词少用**：`真·xxx`（网梗前缀）、`不要钱 / 免费 / 白赚`（形容「几乎零成本」）、`命门 / 必踩`（夸大某处的重要性）、`雷打不动`（夸大「绝对不变」）——分别改成「真正 xxx」「没有额外的成本 / 总量持平」「最容易写错的地方 / 容易踩到的坑」「一直 / 始终不变」（若前文已说「一直是 X」可直接删掉）。
    - **太随意的口语动词收一收**：「它的玩法是」→「它的核心思想是」，「（上一章）也留了话」→「也提到」，「捏进一个栈」→「揉进一个栈」，「坐实了」→「印证 / 证实」，「伺候 Post-LN（的毛病）」→「稳住 / 应对」，「才能逼出（毛病 / 不稳）」→「才能暴露出 / 触发」，「把 X 救回来」→「让 X 重新 work / 恢复正常」。
    - **暴力 / 江湖气的夸张少用**：`最猛的一刀就砍在这儿`、`大改`（形容某处改动大）、`一条龙走完`（形容流程连贯）、`加一发`（当量词用）——分别改成「改动最大的就在这儿」「换装 / 改动最大」「从头到尾走一遍」「加一项」等平实说法。
...
- **术语统一**（这些词在多章里反复被同向替换，下笔时直接用右栏，省得审阅时再改）：
  ...
  - **指「架构层面的小改良」时别用「微调」 → 改良 / 改进**：「微调」在 LLM 语境里几乎专指 fine-tuning（SFT / LoRA，有专门章节讲），拿它去指代 QK-norm、去 bias、RoPE 外推这类**模型结构上的小改动**会和那个核心术语撞车、误导读者。这类"对标准结构做的小手术"统一说「改良 / 改进 / 小改」，把「微调」一词留给 fine-tuning。
