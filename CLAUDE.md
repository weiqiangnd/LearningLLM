...
- **inline `$...$` 含方括号 `[ ]` 时，所有下标 `_` 都要写成 `\_`**——单独的简单 inline math（如 `$V_\phi$`、`$\pi_\theta$`）一切正常；但只要 inline 公式里出现 `[` 或 `]`（无论是 `\left[ ... \right]`、`\mathbb{E}[X]` 还是裸方括号），markdown emphasis 解析就会**漏过 `$` 边界**，把里头的 `_X` 当成 italic 起点处理——结果整段 fallback 成纯文本、所有下标全没。例如 `$\mathcal{L}_V(\phi) = \mathbb{E}\left[(V_\phi(s_t) - G_t)^2\right]$` 渲染成 `\mathcal{L}V(\phi) = \mathbb{E}\left[(V\phi(s_t) - G_t)^2\right]`，下标 `_V`、`_\phi`、`_t` 全被吃掉；改写成 `$\mathcal{L}\_V(\phi) = \mathbb{E}\left[(V\_\phi(s\_t) - G\_t)^2\right]$` 即可恢复。同一段内简单 inline math（无 `[]`）可继续裸写 `_`，不强制全转义。**实在懒得逐个 `\_` 的话，把这条公式抽到 `$$...$$` 块级（独占一行、前后空行）也能绕开**，因为 markdown emphasis 不跨越块级数学边界。`$$` 块级公式则一直没这个问题——里头的 `_` 不需要转义。
