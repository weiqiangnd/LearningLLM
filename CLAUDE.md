...
- **导出宽度按信息密度选**：**默认 `-w 1800`**（绝大多数图都用它——多容器 / 并排矩阵 / 长 sublabel / 节点不算少 / 带斜排列头或图例的，统统走这档）；只有**节点非常少、文本很稀疏**（≤ 6 个节点，且没有斜排列头、图例这类细节）才降到 `-w 1440`；**节点非常多 / 极宽的横向流程**（密集矩阵、>15 节点、超长并排）可以**超过 1800**（如 2000–2200，按内容实际占用范围调）。这套数值是按 retina 屏和 PDF 打印需要的实际像素定的，直接选最终值传给 `rsvg-convert -w`，不用再乘倍率。
