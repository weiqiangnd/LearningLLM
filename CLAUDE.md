...
- **同一行不要出现 2 个以上裸 `~`**——GitHub 用的 cmark-gfm 的 strikethrough 扩展接受**单个** `~` 作为分隔符（不只是规范里的 `~~`），凡同一行有偶数个裸 `~` 都会两两配对、把中间内容渲成中划线；奇数个则前两两配对、最后一个被吞。典型踩坑：用 `~` 当范围号（`2~4 分钟`、`7B ~ 70B`、`0.1 ~ 0.3`）或近似前缀（表格里一行写 `| ~2 | ~50 小时 |`）——`2~4` 会被吃成 `24`、整段加粗内容被划掉。**约定**：范围号一律用 en dash `–`（`2–4 分钟`、`7B–70B`、`0.1–0.3`），近似值用 `≈N`（`≈50 小时`），孤立单 `~`（找不到配对，如 `~16 GB` 整行只有一个）可以保留但不推荐——以后插入新内容凑成偶数就翻车了，最稳还是统一替换掉。`~~text~~`（真的想划掉）不在此列。**排查**：下面这段（跳过 inline code 与 `~~`）应无输出：
      ...
      for p in sorted(pathlib.Path('src').glob('*.md')):
          for i, l in enumerate(p.read_text().splitlines(), 1):
              ...
              if s.count('~') >= 2:
                  print(f"{p}:{i}: {l}")
      PY
- **inline `$...$` 里出现 `}_<letter>` 这种"右花括号紧跟下标 + 字母"的形态，且整段 inline math 内含方括号 `[ ]` 时，要把这个 `_` 转义成 `\_`**——原因是 CommonMark 把 `}` 视作标点：`}_V`（或 `}_t`、`}_\phi`）刚好同时满足"左 flanking"和"前接标点可开启 emphasis"两个条件，而整段公式里的 `[` / `]` 又提供了配对 `_` 让 italics 真的成型——结果整段 fallback 成纯文本、所有下标全没。**典型踩坑**：`$\mathcal{L}_V(\phi) = \mathbb{E}\left[(V_\phi(s_t) - G_t)^2\right]$` 里第一个 `_V`（前面是 `}`、后面是大写字母）就是触发点——渲染成 `\mathcal{L}V(\phi) = \mathbb{E}\left[(V\phi(s_t) - G_t)^2\right]`，下标全被吃掉；把首个 `_V` 写成 `\_V` 即可恢复（剩下的 `_\phi`、`_t` 不强制转义）。**其余形态都不必转义**：单独的简单 inline math（如 `$V_\phi$`、`$\pi_\theta$`）、含 `[...]` 但下标前缀是反斜杠或字母（`\nabla_\theta`、`\sum_a`、`G_t`、`s_t`、`\pi_\theta`）的公式，实测都能正确渲染。**实在不放心 / 公式特别长**：把这条公式抽到 `$$...$$` 块级（独占一行、前后空行）也能绕开，因为 markdown emphasis 不跨越块级数学边界。`$$` 块级公式则一直没这个问题——里头的 `_` 不需要转义。
...
### list bullet 内嵌围栏代码块避坑

**list bullet 里不要直接套三反引号围栏**——markdown-it 在「围栏缩进恰好等于 list 内容列」的边界下会把三反引号当 inline code span 分隔符，整段压成 `<p><code>...</code></p>`，PDF 里换行全没。改用 **6 空格缩进的 indented code block**（= list 内容列 2 + 标准缩进 4），CommonMark 严格保证识别为 `<pre><code>`。
