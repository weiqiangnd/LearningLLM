...
# 可能已经把 dist/ 从历史里抹掉 —— dist/ 就真的没了。
# 改放到仓库父目录（同一持久化磁盘，且不受 tmpfs 容量上限影响）。
BACKUP_DIR=$(mktemp -d -p "$(dirname "$REPO_ROOT")" .prune-dist-backup-XXXXXX)

# 异常退出时把备份路径打到 stderr，避免半途崩溃后用户找不到备份。
# 正常路径走到末尾会主动删掉备份目录并解除 trap。
trap 'echo "⚠️  脚本异常退出，dist/ 备份仍在：$BACKUP_DIR" >&2' ERR
...
# 9. 校验备份与还原一致，一致则删除备份
#####################################################################
echo "==> 校验备份 $BACKUP_DIR/dist 与仓库 dist/ 是否一致"
if diff -rq "$BACKUP_DIR/dist" dist >/dev/null; then
  echo "    一致，删除备份目录"
  rm -rf "$BACKUP_DIR"
