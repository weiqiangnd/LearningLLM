...
- [五、实战：参数对比实验](#五实战参数对比实验)
  - [5.0 准备工作：复用第一章的加载流程 + 一个 chat() 辅助函数](#50-准备工作复用第一章的加载流程--一个-chat-辅助函数)
...
- **`tokenizer.json` + `tokenizer_config.json`**：tokenizer 的全部信息——词表、BPE 合并规则、特殊 token、chat template（一段 Jinja 模板，定义如何把 messages 拼成训练时见过的格式）。第三章会专门讲。
...
- **logits**：模型为词表中**每一个**可能的下一个 token 算出的"原始评分"。形状 `[batch, seq_len, vocab_size]`，是任意实数（可以为负）。我们只关心**最后一个位置**的 logits 切片——也就是"看完到目前为止的所有上下文，下一个 token 应该是什么"。
- **词表大小 V（vocab size）**：模型一共"认识"多少种 token。Qwen3 的词表约 152k。
- **softmax**：把 logits（任意实数）映射成 0~1 的概率分布，下一节给公式。
...
其中 $\ell_i$ 是第 $i$ 个 token 的 logit。引入温度（temperature）$T$ 后是：
...
### 5.0 准备工作：复用第一章的加载流程 + 一个 chat() 辅助函数

**Cell 0 / 1 / 2** 与第一章完全一致：GPU 自检、`%%capture` 装依赖、4-bit 加载 Qwen3-8B 与 tokenizer——不再赘述。下面把 **Cell 3 ~ Cell 5** 三段铺垫代码依次贴出来。
