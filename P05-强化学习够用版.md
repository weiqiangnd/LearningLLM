...
- [五、Policy Gradient 定理](#五policy-gradient-定理)
  - [为什么策略梯度是这个形式：log-derivative trick](#为什么策略梯度是这个形式log-derivative-trick)
...
这条假设是动态规划（DP）和绝大多数 RL 算法的根基——正因为"未来只依赖现在"，价值函数 $V(s)$ / $Q(s, a)$ 才能被定义在单个状态/状态-动作对上、Bellman 方程才能写成递推形式、policy 才能写成 $\pi(a \mid s)$ 而不必依赖整条历史。一旦马尔可夫性不成立（部分可观测、有长程依赖等），常见的补救是把"最近若干帧"打包成一个新的"状态"，让打包后的新状态重新满足马尔可夫性，再套这套语言。
...
—— **状态价值**：从 $s$ 起，按 $\pi$ 行动，期望能拿多少累计回报。逐项拆开看：
...
一句话： $V^\pi(s)$ = "我现在处在 $s$ ，从这里开始一直按 $\pi$ 走下去，期望能攒到多少总回报"。

$$
Q^\pi(s, a) = \mathbb{E}\_\pi \left[ G_t \mid s_t = s, a_t = a \right]
$$

—— **动作价值**：在 $s$ **先选定** $a$ 这一步、之后按 $\pi$ 走，期望能拿多少累计回报。和 $V$ 的差别只在条件多了一项 $a_t = a$ ：
...
### 为什么策略梯度是这个形式：log-derivative trick
...
把它套到我们要证的命题上。用**塔式期望**把对整条 $\tau$ 的期望拆成两层——先条件在"截止到 $t$ 之前的历史" $h_{t-1} = (s_0, a_0, r_0, \ldots, a_{t-1}, r_{t-1}, s_t)$ 上，再对 $a_t$ 取期望。给定 $h_{t-1}$ 后， $s_t$ 和"过去奖励" $\sum_{t' \lt t} \gamma^{t'} r_{t'}$ 都是**常数**（不依赖 $a_t$ ），可以提到内层期望外面：
...
为什么能省？两种常见处理：（a）LLM 对齐里 $\gamma = 1$ （episode 短、终止时一次给齐奖励），所以 $\gamma^t \equiv 1$ ，严格与简写完全一致；（b）即使 $\gamma \lt 1$ ，工程上常把 $\gamma^t$ 因子省去——相当于改成"undiscounted average reward"或"每步同等重要"的近似目标，理论上是有偏的（更新规则不再精确优化原始 $J$ ），但实战表现稳定、公式更简洁，社区已经默认这种写法。Sutton & Barto 第 13 章对这个 $\gamma^t$ 因子的讨论比较细致（参见 2nd ed. §13.2 与 §13.6 的相关讨论），想看推导细节可以去翻；本章按社区实战写法继续。这一步小细节不影响理解，核心结构始终是"对 $\log \pi$ 求梯度，乘上一个动作的打分"。后面 baseline / advantage / PPO 的修补也都建立在这个简化版上。
