...
进一步，训练里几乎只用 **log-softmax**——直接得到 $\log p_i$ ，配合后面要讲的"取对数 + 取负"的损失。从上面减完最大值的 softmax 出发，对两边取 log，再用 $\log \frac{a}{b} = \log a - \log b$ 与 $\log \exp(x) = x$ 一路化简：

$$
\begin{aligned}
\log \text{softmax}(\ell)\_i
&= \log \frac{\exp(\ell_i - m)}{\sum_j \exp(\ell_j - m)} \\
&= \log \exp(\ell_i - m) - \log \sum_j \exp(\ell_j - m) \\
&= \ell_i - m - \log \sum_j \exp(\ell_j - m)
\end{aligned}
$$
