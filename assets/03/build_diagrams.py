...
def diagram_pipeline():
    ...
    # Stop the arrowhead ~8px above the Embedding box top edge so the tip
    # sits just outside the box instead of overlapping the「Embedding」label.
    sx, sy = 1150, cy - h / 2
    ex, ey = 715, cy - h / 2 - 8
    midy = 132
...
def diagram_granularity():
    ...
    box_left = 60
    tokens_x = 284          # where token boxes start (clears the widened name tag)
    ...
    for (y, name, nsub, cf, cb, en, zh, trade1, trade2) in rows:
        ...
        # left name tag — widened so the longest label「子词 subword（BPE）」keeps
        # comfortable left/right padding inside the colored tag (was 170, too tight)
        tag_w = 190
        tag_cx = box_left + 14 + tag_w / 2
        p.append(f'<rect x="{box_left+14}" y="{y-26}" width="{tag_w}" height="52" rx="9" '
                 f'fill="{cf}" stroke="{cb}" stroke-width="2"/>')
        p.append(f'<text x="{tag_cx}" y="{y-3}" text-anchor="middle" class="lbl">{esc(name)}</text>')
        p.append(f'<text x="{tag_cx}" y="{y+17}" text-anchor="middle" class="small">{esc(nsub)}</text>')
...
def architecture_map(highlight, out_path, hl_caption):
    ...
    p.append(f'<text x="{W/2}" y="64" text-anchor="middle" class="sub" '
             f'font-size="14">{esc(hl_caption)}</text>')
