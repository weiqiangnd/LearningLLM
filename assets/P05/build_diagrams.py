...
FONT = "'Noto Sans CJK SC', -apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC', 'Hiragino Sans GB', 'Microsoft YaHei', sans-serif"
...
def write_svg(path: Path, body: str, viewbox: str):
  ...
  <style>
    ...
    .formula {{ font-size: 15px; fill: {TXT}; font-style: italic; }}
    .muted {{ fill: {MUTED}; }}
  </style>
"""
    defs = """
...
def diagram_baseline_variance():
    ...
    def panel(x0, y0, pw, ph, ys, ymin, ymax, title, fill, border, mean_color, mean_val):
        ...
        # Park the mean label in the empty top-right corner of the box (the
        # dashed line itself can sit in a dense scatter band — e.g. the right
        # panel's mean=0 runs through the thickest cluster — so labelling it
        # on the line would collide with data points). A short colour swatch
        # ties the corner label back to the dashed mean line.
        parts.append(f'<line x1="{x0+pw-118}" y1="{y0+14}" x2="{x0+pw-94}" y2="{y0+14}" stroke="{mean_color}" stroke-width="2" stroke-dasharray="6,4"/>')
        parts.append(f'<text x="{x0+pw-86}" y="{y0+19}" class="small" fill="{mean_color}">{label}</text>')
        # scatter
...
def diagram_pg_pipeline():
    ...
    cols_x = [105, 284, 480, 685, 895, 1095]
    # node ① 的 label「① 采样 N 条 rollout」含 CJK + 拉丁词，Noto 下偏宽，
    # 140 会让首尾字符顶到框边，加宽到 168（两侧各留 ~35px 与邻框间隙）。
    widths = [120, 168, 155, 175, 145, 170]
    labels = [
