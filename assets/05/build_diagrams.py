...
def diagram_attention():
    ...
    hy0, dh = 120, 92
    hs = ["h₁", "h₂", "h₃", "h₄"]
    p.append(f'<text x="{hx}" y="{hy0-50}" text-anchor="middle" class="lbl" fill="{BLUE_B}">编码器隐藏态</text>')
    hcy = []
    ...
    # step 1: score column
    scx = 440
    p.append(f'<text x="{scx}" y="{hy0-50}" text-anchor="middle" class="lbl" fill="{ORANGE_B}">① 打分 eₜⱼ</text>')
    p.append(f'<text x="{scx}" y="{hy0-30}" text-anchor="middle" class="small">align(sₜ₋₁, hⱼ)</text>')
    raw = ["0.4", "2.1", "0.7", "-0.3"]
    ...
    wx = 660
    p.append(f'<text x="{wx}" y="{hy0-50}" text-anchor="middle" class="lbl" fill="{PURPLE_B}">② softmax</text>')
    p.append(f'<text x="{wx}" y="{hy0-30}" text-anchor="middle" class="small">αₜⱼ（和为 1）</text>')
    ...
    p.append(f'<text x="{cx}" y="{hy0-50}" text-anchor="middle" class="lbl" fill="{RED_B}">③ 加权求和</text>')
    p.append(f'<text x="{cx}" y="{hy0-30}" text-anchor="middle" class="small">cₜ = Σ αₜⱼ hⱼ</text>')
    for i, cy in enumerate(hcy):
...
# ============================================================
# Diagram 6: 实战代码流水线 —— Cell 3~8 怎么拼成一个带 attention 的 seq2seq
# ============================================================
def diagram_code_pipeline():
    W, H = 1840, 1000
    p = [f'<text x="{W/2}" y="46" text-anchor="middle" class="title">'
         f'实战代码流水线：Cell 3–8 怎么拼成一个带 attention 的 seq2seq</text>']

    def mbox(x, y, w, h, header, lines, fill, border, hcolor=None):
        """方法框：粗体 header（Cell 名 + 方法签名）+ 若干 mono/small 行。
        lines 里每项是 (文本, 类名)，文本为 '' 表示一个空行占位。"""
        hcolor = hcolor or border
        out = [f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="12" '
               f'fill="{fill}" stroke="{border}" stroke-width="2"/>']
        out.append(f'<text x="{x+16}" y="{y+27}" class="lbl" fill="{hcolor}">{esc(header)}</text>')
        yy = y + 51
        for text, cls in lines:
            if text:
                out.append(f'<text x="{x+18}" y="{yy}" class="{cls}">{esc(text)}</text>')
            yy += 21
        return out

    # ---------- Cell 3: 数据 ----------
    c3x, c3y, c3w, c3h = 56, 108, 322, 348
    p += mbox(c3x, c3y, c3w, c3h, "Cell 3 · make_batch()", [
        ("构造 toy 数据：数字串 → 其逆序", "small"),
        ("", "small"),
        ("src      [B, S]", "mono"),
        ("  编码器输入（数字, 右补 PAD）", "small"),
        ("src_len  [B]", "mono"),
        ("  每条真实长度（不含 PAD）", "small"),
        ("tgt_in   [B, S+1]", "mono"),
        ("  解码器输入：SOS + 逆序", "small"),
        ...
        ("  目标：逆序 + EOS", "small"),
        ("", "small"),
        ("VOCAB=13：PAD/SOS/EOS + 0–9", "small"),
        ("tgt_in / tgt_out 错位一格 = teacher forcing", "small"),
    ], TEAL_F, TEAL_B)

    # ---------- Cell 7 容器：Seq2Seq.forward ----------
    sx, sy, sw, sh = 430, 96, 1354, 540
    p.append(f'<rect x="{sx}" y="{sy}" width="{sw}" height="{sh}" rx="16" '
             f'fill="#faf5ff" stroke="{PURPLE_B}" stroke-width="2.4"/>')
    p.append(f'<text x="{sx+20}" y="{sy+30}" class="lbl" fill="{PURPLE_B}">'
             f'Cell 7 · class Seq2Seq.forward(src, src_len, tgt_in)</text>')
    p.append(f'<text x="{sx+20}" y="{sy+52}" class="sub">'
             f'组装 Encoder + 逐步 Decoder，按目标长度 T 循环，返回 (logits, attn)</text>')

    # ---------- Cell 4: Encoder ----------
    e4x, e4y, e4w, e4h = 458, 198, 300, 208
    p += mbox(e4x, e4y, e4w, e4h, "Cell 4 · Encoder.forward", [
        ("(src, src_len)", "mono"),
        ("Embedding(V, emb=32)", "mono"),
        ("→ GRU(emb, hid=96)", "mono"),
        ("pack_padded：跳过 PAD", "small"),
        ("———— 输出 ————", "small"),
        ("H   [B, S, hid]", "mono"),
        ("h_n [1, B, hid] → s₀", "mono"),
    ], BLUE_F, BLUE_B)
    ...
    # ---------- 解码循环容器 ----------
    lx, ly, lw, lh = 792, 176, 970, 410
    p.append(f'<rect x="{lx}" y="{ly}" width="{lw}" height="{lh}" rx="14" '
             f'fill="#ffffff" stroke="{SUB}" stroke-width="2" stroke-dasharray="9,5"/>')
    p.append(f'<text x="{lx+18}" y="{ly+26}" class="lbl" fill="{TXT}">'
             f'for t = 1 … T :  逐步解码（teacher forcing 喂真值 tgt_in[:, t]）</text>')

    # Cell 5: Attention
    a5x, a5y, a5w, a5h = 816, 218, 890, 150
    p += mbox(a5x, a5y, a5w, a5h, "Cell 5 · BahdanauAttention(sₜ₋₁, H, mask)", [
        ("eₜⱼ = vᵀ tanh(W·sₜ₋₁ + U·hⱼ)        给每个源位置 j 打分", "mono"),
        ("屏蔽 PAD(mask=False → −∞)，再 αₜ = softmax(eₜ)", "mono"),
        ("→ context [B, hid] = Σⱼ αₜⱼ·hⱼ      ·      alpha αₜ [B, S]", "mono"),
    ], ORANGE_F, ORANGE_B)

    # Cell 6: Decoder.step
    d6x, d6y, d6w, d6h = 816, 406, 884, 164
    p += mbox(d6x, d6y, d6w, d6h, "Cell 6 · Decoder.step(yₜ₋₁, sₜ₋₁, H, mask)", [
        ("内部调用 Cell 5 的 attention 拿到 context", "small"),
        ("sₜ = GRUCell( [ emb(yₜ₋₁) ; context ] , sₜ₋₁ )      更新隐藏", "mono"),
        ("logitsₜ = Linear( [ sₜ ; context ] ) → [B, V]      词表打分", "mono"),
        ("use_attention=False：context ≡ s₀（编码器末步）= baseline 瓶颈", "small"),
    ], GREEN_F, GREEN_B)

    # ---------- 容器内部数据流箭头 ----------
    # Cell3 → Encoder（src, src_len）
    p += arrow(c3x + c3w, 286, e4x - 4, 286, marker="aTeal", color=TEAL_B, width=2.2)
    p.append(f'<text x="{(c3x+c3w+e4x)/2}" y="276" text-anchor="middle" class="small" fill="{TEAL_B}">src · src_len</text>')
    # Encoder → Attention（H）
    p += arrow(e4x + e4w, 300, a5x - 4, 300, marker="aBlue", color=BLUE_B, width=2.2)
    p.append(f'<text x="{(e4x+e4w+a5x)/2}" y="290" text-anchor="middle" class="mono" fill="{BLUE_B}">H</text>')
    # Encoder → Decoder（h_n → s₀，初始 sₜ₋₁）
    p.append(f'<path d="M {e4x+e4w} 372 C 786 372, 792 470, {d6x-4} 482" '
             f'fill="none" stroke="{BLUE_B}" stroke-width="2" marker-end="url(#aBlue)"/>')
    p.append(f'<text x="770" y="398" text-anchor="middle" class="small" fill="{BLUE_B}">s₀</text>')
    # Cell3 → Decoder（tgt_in[:,t] = yₜ₋₁）
    p.append(f'<path d="M {c3x+c3w} 440 C 600 452, 700 500, {d6x-4} 512" '
             f'fill="none" stroke="{TEAL_B}" stroke-width="1.8" stroke-dasharray="6,4" marker-end="url(#aTeal)"/>')
    p.append(f'<text x="500" y="436" text-anchor="middle" class="small" fill="{TEAL_B}">tgt_in[:, t] → yₜ₋₁</text>')
    # Attention → Decoder（context）
    ctx_x = 1150
    p += arrow(ctx_x, a5y + a5h, ctx_x, d6y - 2, marker="aOrange", color=ORANGE_B, width=2.4)
    p.append(f'<text x="{ctx_x+12}" y="{(a5y+a5h+d6y)/2+5}" class="mono" fill="{ORANGE_B}">context</text>')
    # Decoder sₜ 回流 → 下一步 sₜ₋₁
    p.append(f'<path d="M {d6x+d6w} 470 C 1742 470, 1742 300, {a5x+a5w+2} 300" '
             f'fill="none" stroke="{GREEN_B}" stroke-width="2" marker-end="url(#aGreen)"/>')
    p.append(f'<text x="1752" y="388" text-anchor="middle" class="small" fill="{GREEN_B}" '
             f'transform="rotate(-90 1752 388)">sₜ → 下一步的 sₜ₋₁</text>')

    # Seq2Seq 返回值（容器底部）
    p.append(f'<text x="{sx+sw/2}" y="618" text-anchor="middle" class="lbl" fill="{PURPLE_B}">'
             f'forward 返回 → logits [B, T, V]   ·   attn [B, T, S]</text>')

    # ---------- Cell 7: train() ----------
    tx, ty, tw, th = 430, 688, 624, 180
    p += mbox(tx, ty, tw, th, "Cell 7 · train(model, steps)", [
        ("每步：logits [B,T,V] 对齐 tgt_out [B,T]", "mono"),
        ("loss = F.cross_entropy(…, ignore_index=PAD)", "mono"),
        ("  PAD 位置不计损失", "small"),
        ("Adam：loss.backward() → opt.step()", "mono"),
        ("（seq_accuracy：整条序列全对才算对）", "small"),
    ], RED_F, RED_B)

    # ---------- Cell 8: 训练入口 ----------
    gx, gy, gw, gh = 1110, 688, 674, 180
    p += mbox(gx, gy, gw, gh, "Cell 8 · 训练两个模型对比", [
        ("model_attn  = Seq2Seq(…, use_attention=True)", "mono"),
        ("model_plain = Seq2Seq(…, use_attention=False)", "mono"),
        ("  后者 context 固定 = baseline 瓶颈", "small"),
        ("两者各 train(steps=1500)", "mono"),
        ("再比「准确率 vs 源句长度」曲线（Cell 10）", "small"),
    ], GRAY_F, GRAY_B, hcolor=TXT)

    # 容器 → train（logits）
    p += arrow(740, sy + sh, 740, ty - 2, marker="aPurple", color=PURPLE_B, width=2.2)
    p.append(f'<text x="752" y="{(sy+sh+ty)/2+5}" class="mono" fill="{PURPLE_B}">logits</text>')
    # Cell3 → train（tgt_out）
    p.append(f'<path d="M {c3x+90} {c3y+c3h} C {c3x+90} 720, 300 778, {tx-2} 778" '
             f'fill="none" stroke="{TEAL_B}" stroke-width="1.8" marker-end="url(#aTeal)"/>')
    p.append(f'<text x="250" y="690" text-anchor="middle" class="small" fill="{TEAL_B}">tgt_out [B, T]</text>')
    # train → 容器（参数更新，反向虚线）
    p.append(f'<path d="M 1000 {ty} L 1000 {sy+sh+2}" '
             f'fill="none" stroke="{RED_B}" stroke-width="1.8" stroke-dasharray="6,4" marker-end="url(#aRed)"/>')
    p.append(f'<text x="1012" y="{(ty+sy+sh)/2+5}" class="small" fill="{RED_B}">梯度更新 Seq2Seq 全部参数</text>')
    # Cell8 → train（调用）
    p += arrow(gx - 2, 778, tx + tw + 4, 778, marker="aGray", color=GRAY_B, width=2)
    p.append(f'<text x="{(gx+tx+tw)/2}" y="768" text-anchor="middle" class="small" fill="{SUB}">调用 train()</text>')

    # ---------- 底部记号说明带 ----------
    by, bh2 = 898, 76
    p.append(f'<rect x="56" y="{by}" width="{W-112}" height="{bh2}" rx="12" '
             f'fill="{GRAY_F}" stroke="{SUB}" stroke-width="1.6"/>')
    p.append(f'<text x="{W/2}" y="{by+30}" text-anchor="middle" class="lbl" fill="{TXT}">'
             f'形状记号：B = 一批样本数 · S = 源序列长(含 PAD) · T = 目标步数(= S+1) · emb = 32 · hid = 96 · V = VOCAB = 13</text>')
    p.append(f'<text x="{W/2}" y="{by+58}" text-anchor="middle" class="sub">'
             f'mask [B, S]：真实 token = True、PAD = False —— attention 用它把 PAD 位置打 −∞，softmax 后权重 ≈ 0</text>')

    write_svg(ASSETS / "code-pipeline.svg", "\n".join(p), f"0 0 {W} {H}")
...
if __name__ == "__main__":
    ...
    diagram_alignment()
    diagram_timing()
    diagram_code_pipeline()
    print("wrote 6 SVGs to", ASSETS)
