...
ipynb 里的实战部分按本章正文的概念顺序展开：先在二次型山谷上画 SGD / Momentum / Adam 三种轨迹，再用最小数字把 §3.1 / §4.4 的两个手算例子跑一遍并对照解析稳态；接着手写一遍 Adam、跟 PyTorch 内置实现逐元素对齐验证公式没看错；然后在 `make_moons` 上做三件 lr / 调度相关的实验——lr 太大 / 适中 / 太小三种取值的 loss 曲线、有无 warmup 在大初始化 + 大 lr 下的对照、以及 SGD / Momentum / Adam / AdamW 四种优化器同条件下的训练曲线对比；最后给一份 warmup + cosine 调度的 lr 曲线和一份"AdamW + warmup + cosine"完整训练循环模板，可直接套到任何 LLM 训练脚本里。
