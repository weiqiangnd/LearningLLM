...
下图把这件事拆成三栏：8 个参数的梯度量级横跨 4 个数量级（第 a 栏，从 10 到 0.002），SGD 的实际 update 严格正比于梯度（第 b 栏，跨 5 个数量级），而 Adam 的 update 在每个参数上都被 $\sqrt{\hat v_t} + \epsilon$ 归一化到 $\approx \eta$ ，几乎齐平（第 c 栏）：
